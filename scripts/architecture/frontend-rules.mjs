...
// 不预留余量。
// 合并后重测(该侧自己的 merge):上面两组理由分属这条分支的三个 change(权限决定、Shell 生命周期、
// 搜索取消与预算)与 main 侧的 Skill Evolution,落在互不相交的文件上。合并树实测 27405,不是
// 26515+890 也不是 25116+2289——相加会把两边共有的基线算两遍。按实测记,不留余量。
// 上调理由(本次合并 redesign-unified-workbench-ui 分支与 origin/main):两侧各自独立累积到 24330
// 与 27405,同样互不相交(本分支的工作台重构 vs. main 侧的权限/Shell/搜索/Skill Evolution),按
// 惯例在合并树上重测而不是相加。合并树实测 27594(不是 24330+27405,相加会把两边共有的基线算
// 两遍),按实测记,不留余量。
const SUBTREE_LINE_BUDGETS = Object.freeze([
  { root: "src/services", budget: 27594, owner: "merge/redesign-unified-workbench-ui" },
]);
