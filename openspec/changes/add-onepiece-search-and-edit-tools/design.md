...
三者均属 ripgrep 生态（BurntSushi），是此类需求的事实标准。但会明显加宽间接依赖树。`software-supply-chain-security` spec 有 `Vulnerable dependency prevention` 与 `Automated dependency maintenance` 两条需求，proposal 中须单列一节说明引入理由与审查结论，不可夹带。
