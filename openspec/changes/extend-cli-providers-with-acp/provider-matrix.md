...
| qoder-cli | 1.1.45，npm `@qoder-ai/qodercli`（`qoder` 与别名 `qodercli` 均在 PATH，同一包） | 同上 | `qoder --acp`：protocol 1、loadSession=true、`qoder-cli 1.1.45`、authMethods `["qodercli-login"]` | PASSED | 同上（5.4 s）；2026-09-07 用户登录后：`session/new` ACCEPTED，prompt 被服务端以额度用尽拒绝（`acp-prompt-rejected`，HTTP 500），模型轮次 BLOCKED（账号额度） | live-verified（仅握手层） |
