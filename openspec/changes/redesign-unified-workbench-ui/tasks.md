...
- [ ] 4.5 Implement redirects from legacy Loops, Mission Control, Work Board, Goals, Evaluation, and Scheduled Tasks routes to the new equivalents. — Explicit known gap, not silently skipped: `parseWorkbenchLocation` now treats old paths like `/workspace/loops` and `/workspace/mission-control` as unsupported ids and falls back to the Sessions default (same fallback as a truly unknown path), rather than issuing a real redirect to `/workspace/runs/loops` etc. A user with an old bookmark or browser-history entry lands on Sessions instead of where they meant to go. Needs its own pass mapping each of the 6 old ids to its new equivalent path before 4.14's one-time hint can be layered on top.
