...
- [ ] 4.2 Define typed secondary routes for Runs Attention/Active/History/Loops/Schedules, Plan Board/Goals, and Quality Evaluations. — `RunsSection`/`PlanSection`/`QualitySection` in `workbench-route.ts`, same file as 4.1 (marking 4.1 alone would be artificial — they're one type model). Path scheme: primary entity id as a path segment (`/runs/active/<runId>`, `/plan/goals/<goalId>`), list/filter-modifying values as query params (`?view=`, `?compare=a,b`) since those can coexist with an absent primary id.
- [ ] 4.3 Replace direct destination conditionals in `main-layout.tsx` with a lazy `DestinationDefinition` registry. — Not started; read the full 503-line file directly first. Sessions is not a peer of the other 5 — it's the entire session workspace apparatus (sidebar/tabs/info-panel/dialogs, dozens of local hooks), while loops/work-board/goals/evaluations/mission-control are near-identical `{visited ? <LazyFeature/> : null}` siblings with heterogeneous componentProps (mission-control needs `onNavigate`, loops needs `onInspect` closing over `notify`/`agentService`/local setState — these can't move into a plain exported array without an adapter). Genuinely entangled with 5.1-5.3 (AppShell/DestinationLayout is what a registry gets iterated *into*) — doing 4.3 fully isolated from that would build a registry nothing consumes yet. Deferred to the same work session as 5.1-5.3 rather than attempted piecemeal now.
...
- [ ] 4.6 Map the legacy scheduled-task dialog-open state to `/runs/schedules` and remove it after compatibility coverage passes. — Confirmed via direct investigation: `scheduledTasksOpen` is a bare `useState(false)` in `main-layout.tsx` (no URL representation at all today) — this task's premise is exactly right, not already partially done.
