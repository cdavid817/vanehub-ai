...
- [ ] 5.13 Implement `draft-only` retention and shell-level navigation protection without serializing secrets. — Done: Settings' three draft-only pages (`shouldRenderPage`-driven retention, in-memory only — nothing is ever written to `localStorage` or the native settings service, so "without serializing secrets" holds by construction, not by redaction). Sessions' composer draft (pre-existing behavior, now classified and documented rather than accidental, see 5.11).
  - **Found, not fixed — two real, currently-silent draft-loss risks inside Runs' own tab switching** (see 5.11's boundary note): `loop-run-controls.tsx`'s `feedback` textarea (`loop-run-controls.tsx:26` — the "continue with feedback" composition box shown when a Loop run is `awaiting-acceptance`) and `scheduled-tasks-panel.tsx`'s inline new-scheduled-task form (`name`/`content`/`agentId`/`frequency` local state) both lose in-progress typed text on any tab switch away from "loops"/"schedules", or on leaving Runs entirely — with no warning and no persistence layer. Deliberately not fixed in this pass: a correct fix needs an architecture decision this change's design does not make (lift state to `RunsDestination`, which only survives internal tab switches and not leaving Runs entirely; lift all the way to `main-layout.tsx`, mirroring Sessions; or a section-scoped `PageLifecyclePolicy` concept that does not exist yet) rather than a quick patch chosen under time pressure. Left as an explicit, evidenced gap for a follow-up task rather than silently papered over.
