...
- FROM: `### Requirement: Scheduled task dialog`
- TO: `### Requirement: Scheduled task management surface`

## MODIFIED Requirements

### Requirement: Scheduled task management surface
The workspace SHALL provide a responsive scheduled-task management surface, hosted as the Scheduled tab of the Automations destination, that lets users create and manage scheduled tasks through the frontend service boundary.

#### Scenario: Open scheduled task dialog
- **WHEN** the user looks for the retired Scheduled Tasks activity entry
- **THEN** the workspace SHALL NOT open a scheduled-task dialog
- **AND** scheduled-task management SHALL be reached through the Scheduled tab of the Automations destination instead

#### Scenario: Open scheduled task management
- **WHEN** the user opens the Scheduled tab of the Automations destination
- **THEN** the workspace SHALL render the scheduled-task list and creation controls inline as page content
- **AND** focus SHALL move to the first interactive control of the surface
- **AND** it SHALL NOT open a modal dialog
...
- **WHEN** the scheduled-task surface is visible
...
- **THEN** the surface SHALL identify the invalid field and keep creation unavailable
...
- **WHEN** the surface refreshes a previously loaded task list
...
- **THEN** the surface SHALL perform the mutation through the frontend service boundary
...
- **THEN** the surface SHALL retain the user's relevant context and display the error without clearing the existing task list

#### Scenario: Keep the surface mounted across tabs
- **WHEN** the user switches to another Automations tab and returns
- **THEN** the scheduled-task surface SHALL keep its loaded list and any in-progress draft
