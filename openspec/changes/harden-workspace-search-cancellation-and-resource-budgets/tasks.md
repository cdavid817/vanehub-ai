...
- [x] 9.1 Add instrumented providers/temporary-tree builders that count visits, opens, bytes, metadata operations, retained candidates, checkpoints, active workers, and queue depth without depending on production timing.
- [x] 9.2 Add performance gates proving candidate/page memory bounds, exact budget stops, cancellation checkpoint bounds, and global/per-workspace admission limits on large synthetic trees.
