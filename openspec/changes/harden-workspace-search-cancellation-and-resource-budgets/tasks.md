...
- [ ] 0.1 Read `AGENTS.md`, `openspec/project.md`, the current `session-project-inspection` and `runtime-performance-governance` specs, and every active change touching files, documents, search, remote inspection, runtime admission, or workspace UI.
- [ ] 0.2 Map current local/remote directory listing, path search, content search, document discovery, cancellation registry, runtime/API `spawn_blocking`, frontend services/adapters, coverage types, cursor format, ignore rules, and tests.
- [ ] 0.3 Run `openspec validate harden-workspace-search-cancellation-and-resource-budgets --strict`; fix change artifacts before code if current main/active changes conflict.
- [ ] 0.4 Add deterministic characterization for the A/B same-id interleaving where A's finish removes B, and for abort/drop leaving a running worker or stale registration.
...
- [ ] 4.2 Count every visited directory/entry and metadata/canonicalization operation before filtering or matching; stop with precise coverage when a limit is reached.
- [ ] 4.3 Replace full candidate collection/sort with a bounded top-K/heap/selection structure preserving the current stable ordering and tie-break behavior.
- [ ] 4.4 Bound retained candidates independently of returned results and expose instrumentation for tests.
...
- [ ] 4.6 Verify existing search syntax, path normalization, and result DTO compatibility remain unchanged outside explicit coverage additions.
