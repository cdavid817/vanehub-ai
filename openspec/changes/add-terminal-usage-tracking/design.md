...
4. **A 5-second periodic re-read while output is flowing, plus one more read at process exit as a safety net.** The read loop in `terminal_process.rs` already throttles the session-discovery poll against wall-clock time inside the same `Ok(count) =>` arm that receives PTY output; the usage poll reuses that exact pattern with its own longer-lived timer (`TERMINAL_USAGE_POLL_INTERVAL`), so it only does file/DB IO when output is actually happening and at most once per interval. Being tied to output events (not a free-running OS timer) means a session with no output for a while won't refresh until output resumes or the process exits — acceptable since the exit-time read is unconditional and always catches up.
   - Alternative considered: a free-running timer thread independent of PTY output. Rejected as unnecessary complexity for this change — a session producing no output also has nothing new to report, and the exit-time read guarantees eventual correctness regardless.
