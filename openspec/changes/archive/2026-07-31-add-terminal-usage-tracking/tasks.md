...
- [x] 6.7 Manual end-to-end smoke test on the user's own machine. First attempt (opencode) surfaced a real bug: the periodic poll was gated on PTY output arrival, so it stopped checking once the CLI went idle waiting for the next prompt — confirmed directly from the unified log and opencode's own SQLite row (`session` row existed several seconds before its token columns were populated, with no output in between). Fixed by moving the poll to an independent timer thread (see design.md Decision 4, commit `be5b7f8`); user proceeded to request archiving without further issues reported.
- [ ] 6.8 Gemini CLI terminal-mode usage is out of scope for this change (see proposal Non-Goals) and remains unimplemented; needs a real authenticated `chats/*.jsonl` sample before a follow-up change can add it. Intentionally left unchecked — not a blocker for this change, tracked as a follow-up.
