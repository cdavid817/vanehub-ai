# custom-instructions Delta Specification

## REMOVED Requirements

### Requirement: Custom instructions configuration and persistence
**Reason**: This requirement defined custom instructions as host-level values with a single enablement toggle governing OnePiece and a fixed CLI list together, persisted through the shared application settings model. This change replaces that model with scoped, inherited policy persisted through the dedicated revisioned personalization service, so the previous scenarios — including "One toggle governs every agent" — no longer describe the system.

**Migration**: Replaced by "Scoped custom instructions configuration and persistence" below. Existing host-level field values and the enablement toggle migrate into the global policy scope, so an existing installation keeps its current text and effective behavior.

### Requirement: Custom instructions system-prompt section assembly
**Reason**: Section assembly previously depended on a single enabled/disabled toggle and one undifferentiated pair of fields. Assembly now consumes resolved instruction segments carrying scope and merge-mode provenance, and the failure path is defined by fail-closed snapshot resolution rather than a settings lookup.

**Migration**: Replaced by "Resolved custom instructions system-prompt section assembly" below, which preserves the response-style-before-about-you ordering and the no-empty-placeholder behavior.

### Requirement: Web runtime custom instructions parity
**Reason**: The Web/mock parity contract covered only settings persistence and loading for a host-level toggle. It must now cover scoped policy, inheritance, expected-revision conflicts, effective preview, and session modes.

**Migration**: Replaced by "Web runtime scoped custom instructions parity" below, which retains the existing rule that fixed simulated responses need not vary with assembled prompt content.

### Requirement: Custom instructions CLI prompt injection
**Reason**: CLI coverage was defined as a fixed agent list (`claude-code`, `codex-cli`, `gemini-cli`, `opencode`) reading one host-level enablement toggle. Coverage now derives from the stable Agent registry and runtime capability metadata, and each turn captures its own effective snapshot.

**Migration**: Replaced by "Resolved custom instructions CLI prompt injection" below, which preserves the ordering guarantee before Prompt-Hook-assembled content and the rule that Prompt Hook template variables keep the user's original input.
