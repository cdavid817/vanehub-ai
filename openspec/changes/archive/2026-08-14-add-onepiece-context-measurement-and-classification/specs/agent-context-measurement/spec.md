...
### Requirement: Non-mutating shadow decision
The system SHALL evaluate a versioned token-aware compaction decision from each snapshot in shadow mode while this capability is introduced, and SHALL NOT use that result to mutate request content or replace the active compaction trigger.

#### Scenario: Compare shadow and active decisions
- **WHEN** a request snapshot has known capacity and sufficient measurement data
- **THEN** the system SHALL calculate whether the versioned shadow policy would compact
- **AND** it SHALL compare that decision with the active character-count decision

#### Scenario: Shadow capacity is unknown
- **WHEN** model capacity is unknown
- **THEN** the shadow decision SHALL report `insufficient-capacity-metadata`
- **AND** the active request flow SHALL continue to use existing behavior

#### Scenario: Shadow analysis fails
- **WHEN** snapshot construction, estimation, grouping, or classification fails
- **THEN** the provider request SHALL continue under the existing compaction behavior
