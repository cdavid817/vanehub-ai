...
- `cargo test --manifest-path src-tauri/Cargo.toml`: 87 tests passed, including migration 10, repository atomicity, catalog safety, catalog-ordered multi-select normalization, all provider fresh/resume shapes, interactive snapshot reload, availability isolation, ChatConfig precedence, and next-process semantics.
