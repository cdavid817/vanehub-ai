...
The sidebar SHALL support session navigation without the six bottom tool shortcuts and SHALL provide activity and folder grouping modes.
...
The main content panel SHALL resize with the available workspace area while keeping the bottom composer usable.

#### Scenario: Canvas flexes with panel height
- **WHEN** the workspace panel height changes
- **THEN** the canvas area SHALL flex to fill the remaining main content space without a fixed minimum height forcing overflow
