...
- [x] 10.7 Run `openspec validate improve-workspace-ui-ergonomics --strict` and `openspec validate --specs --strict`. — Re-run fresh before archiving (2026-09-04): `--strict` failed with a real error -- `specs/main-layout-ui/spec.md`'s "Workspace activity bar" MODIFIED block still carried this task's own original pre-authoring content (the old Session/Loops/Scheduled-Tasks activity-bar model), predating `redesign-unified-workbench-ui`'s later, unrelated rebuild of the same requirement into the 5-domain model (Sessions/Projects/Runs/Plan/Quality) on this shared branch. A MODIFIED block replaces the whole requirement, so archiving as authored would have silently reverted 4 scenarios ("Render business destinations", "Identify an entry", "Open grouped capabilities", "Use a narrow height") to describe a UI that no longer exists. Fixed by restating the current main spec's full 11-scenario content verbatim and appending only this change's own genuinely new scenario ("Open documentation from the Help utility entry"), reworded to the current spec's "utility entry" terminology. Re-validated clean.
