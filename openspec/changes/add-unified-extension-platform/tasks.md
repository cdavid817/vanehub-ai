...
- [x] 3.1 Add additive SQLite migrations for publishers, packages, installations, snapshots, dependencies, contributions, runtime generations, operation witnesses, Hook definitions/bindings/executions, authorization rules/rule sets, connector definitions/instances/bindings. The trusted publisher-key table landed with task 2.4 as migration 84, the Developer Mode switch with task 2.5 as migration 85, and snapshots plus installations with task 2.9 as migration 86 — all because the required order puts those tasks first and each needed somewhere to write. Do not add a second table for any of them.
...
- [x] 3.5 Add retention and redaction bounds for Hook executions, runtime diagnostics, and operation witnesses; raw secrets and unrestricted environment data must be impossible to persist.
  No second `runtime_diagnostics` table. Hook executions reuse migration 88's bounds. Operation witnesses gain a `PersistableOperationWitness` the repository takes instead of a raw witness — one constructor, every bound checked: total canonical size, per-collection counts, per-field length, control characters refused rather than stripped, and a schema version (migration 91). Retention never removes evidence for an unfinished operation, a package that is active/rollback/quarantine, or a row a newer build wrote. `SafeExtensionRuntimeDiagnostic` is a host-owned code plus ids plus integer measurements — no string an extension chose can reach it, so redaction has nothing to miss — and it goes out through `platform::logging`. Real runtime producers arrive with Task Groups 4 and 5.
...
- [ ] 10.4.1 Add `connector_credential_transitions` as a durable state machine: `prepared` → `new_stored` → `switched` → `cleanup_pending`/`completed`, recording the predictable new handle **before** the secret is written. Migration 90 stores handles and defines `ConnectorCredentialPort`, and deliberately does **not** implement crash-safe replacement: `write-new → CAS → delete-old` survives an error return only, and a process killed between the credential-store write and the database commit leaves an orphaned secret and an instance pointing at the old handle. Task Group 3 has no connect or execute path, so there was no real replacement to make safe and no honest way to exercise recovery code.
