...
- [x] 2.2 Select and pin the ZIP, Ed25519, hashing, canonicalization, and semantic-version dependencies after license, advisory, maintenance, Windows, streaming-limit, and fuzz/test-vector review. Recorded in `design.md` under "Package security dependencies (Task 2.2)". Only Ed25519 was new: `ed25519-dalek = { version = "3", default-features = false }`, already in the build through `russh` → `ssh-key`, so the addition is one edge in `Cargo.lock`. Canonicalization takes no dependency. Test vectors and fuzz corpora are recorded as obligations of tasks 2.3 and 2.11 rather than treated as satisfied here.
...
- [x] 2.11 Add security tests/fuzz targets for archive parser boundaries, path normalization, signature substitution, revoked keys, stale witnesses, rollback attempts, partial writes, cancellation, and concurrent installs. Landed as `extension_platform::security_tests` and recorded in `design.md` under "The security suite, and what it found (Task 2.11)". No continuous fuzzing harness: `cargo-fuzz` needs a nightly toolchain, a workspace member, and a CI job that do not exist, so what is here is a deterministic mutation corpus, named as such. It found two defects, both fixed — `PortablePackagePath` admitted Unicode bidirectional overrides, and `create_owned_directory` reported a lost creation race as a filesystem error — and one behavior recorded rather than changed. **Cancellation and rollback attempts are not covered here**: cancellation has no machinery until Task Group 4, and refusing a version downgrade is a lifecycle policy rather than a package rule. Both are verified in 4.10.
...
- [ ] 4.10 Add exhaustive state-machine tests, stale operation tests, concurrent enable/disable/reload tests, failure-injection tests at each lifecycle stage, and restart recovery tests. Also owns the two items task 2.11 could not cover: cancellation of an in-flight install, and whether a version downgrade is refused or merely shown. The witness already records the installed version alongside the offered one, so the downgrade is visible; the policy is this task group's to decide.
