...
- [x] 2.1 Extract shared safe-archive, path-normalization, and content-addressed-store primitives into an application-owned `platform` module and route `skills`, the future `skill_registry`, and `extension_platform` through it. The remote Skill Registry change has not started, so its primitives do not exist; the only current implementation is `skills/infrastructure/filesystem/overlay_import.rs`, which another context may not import directly.

  Landed as `platform::archive` (entry-name rule, bounded ZIP inspection and extraction, isolated staging, end-of-archive check) and `platform::content_address` (digest and digest-name rule). Overlay import and the Overlay payload store now call them; characterization tests written first pin that the answer and the operator-visible code did not change. Two findings are recorded in those tests rather than fixed here: `zip = 8.6.0` refuses encrypted and unreadable-method entries before this module's own checks, which makes those checks a guard against a future reader rather than the active one; and the archive entry-name rule is a floor that admits NUL bytes, Windows reserved names, and trailing dots, which is why `PortablePackagePath` still states the stricter rule separately. `extension_platform` consumes these in tasks 2.6 and 2.7, where it first has a package to read.
- [x] 2.2 Select and pin the ZIP, Ed25519, hashing, canonicalization, and semantic-version dependencies after license, advisory, maintenance, Windows, streaming-limit, and fuzz/test-vector review. Recorded in `design.md` under "Package security dependencies (Task 2.2)". Only Ed25519 was new: `ed25519-dalek = { version = "=3.0.0", default-features = false }`, already in the build through `russh` → `ssh-key`, so the addition is one edge in `Cargo.lock`. Exact-pinned to match `zip` and `wasmtime`, which this repository pins because a test characterises the dependency's own behaviour rather than only its API; the direct pin does not reach `curve25519-dalek`, so the committed `Cargo.lock` is the precise lock for the rest of the graph and a Dependabot lockfile pull request is the upgrade-review boundary. Canonicalization takes no dependency. Test vectors and fuzz corpora are recorded as obligations of tasks 2.3 and 2.11 rather than treated as satisfied here.
- [x] 2.3 Implement streamed package hashing, signature-envelope parsing, canonical signed payload construction, publisher-key lookup, key revocation, and deterministic verification diagnostics. Envelope format, signing context, check order, and the four-state `SignatureState` are recorded in `design.md` under "The signature envelope, and what verifying it establishes (Task 2.3)". The manifest claim a signature covers is confirmed against the extracted manifest through the type system rather than by convention: `VerifiedSignature::confirm_manifest` is the only route to the `ConfirmedSignature` a witness may record. Publisher-key *storage* is task 2.4; this task defines the lookup port and the revocation rule only.
- [x] 2.4 Add trusted publisher-key repository/application services for list, preview add, add, revoke, and inspect provenance; store key material/fingerprint according to current secure-storage conventions. Recorded in `design.md` under "Trusted publisher keys (Task 2.4)". No secret is involved — a publisher key verifies signatures and cannot make them — so the store is SQLite (migration 83) and the credential store is not used; the property needed is integrity, not confidentiality. Revocation is permanent in V1 and re-adding a revoked key is refused rather than silently reversing it.
- [x] 2.5 Implement default rejection of unsigned packages and Developer Mode admission only as disabled + Strict with a persistent audited warning. Recorded in `design.md` under "Unsigned content and Developer Mode (Task 2.5)". Developer Mode is a separate audited switch (migration 84), not an eighth capability gate: the seven gates say which parts of the platform are built and switched on, while this says whether content with no provenance may be installed at all. The admission result carries the persistent warning; attaching it to an installation record happens in Task Group 4, where that record first exists.
- [x] 2.6 Implement bounded archive inspection/extraction using the limits in `design.md`; reject traversal, links, devices, duplicate normalized paths, Unicode/case collisions, compression bombs, oversized schemas/results, and undeclared executables. Recorded in `design.md` under "Reading a `.vhext` (Task 2.6)". Two passes: the first reads only what the archive says about itself and runs before extraction, so a package that would be refused is never unpacked. Devices, sockets, and FIFOs are refused through the Unix file-type bits, which the shared `ArchiveEntry` now carries, because ZIP has no entry kind for them. Oversized *results* are deliberately not here: they bound what an extension produces rather than what it ships, so they belong to the runtime trust profile in Task Group 5.
- [x] 2.7 Implement application-owned `quarantine`, content-addressed `packages`, runtime `scratch`, and `sidecars` roots with canonical ownership checks and safe cleanup helpers. Recorded in `design.md` under "The four roots, and what makes them the application's (Task 2.7)". The ownership walk lives in `crate::platform::filesystem::owned` and uses `symlink_metadata` throughout, so a component replaced by a link is refused rather than followed. This also completes the deferral recorded in task 2.1: `create_safe_directory` moved out of the Overlay payload store into the shared primitive, and Skills now calls it with its own wording preserved.
- [x] 2.8 Implement `ExtensionInstallWitness` generation and validation binding package hash, signature state, manifest digest, installed state, dependencies, capability diff, contribution summary, compatibility, and selected trust profile. Recorded in `design.md` under "The install witness (Task 2.8)". Confirmation reports *which* facts moved rather than only that something did, because "the publisher key was revoked" and "try again" are different things to tell a user. The dependency *plan* is supplied by Task Group 4's resolver; what lands here is the shape the witness binds it in and the comparison that refuses a stale one.
- [x] 2.9 Implement atomic snapshot publication and compensated SQLite pointer updates; retain the previous active snapshot on every failure path. Recorded in `design.md` under "Publishing a snapshot across two kinds of storage (Task 2.9)". Content first, pointer last, because unreferenced bytes are collectable and a pointer naming absent content is not. Migration 85 adds `extension_platform_snapshots` and `extension_platform_installations`; task 3.1 must not add a second set.
- [x] 2.10 Implement startup reconciliation for abandoned quarantine, unreferenced snapshots, stale scratch directories, orphan sidecar state, and incomplete transaction journals without deleting user-owned paths. Recorded in `design.md` under "What a restart may clean up (Task 2.10)". Package content is retained when any snapshot *row* names it rather than when a pointer does, because the rollback target is exactly the snapshot nothing points at. Incomplete transaction journals have no separate mechanism: this design has no journal, and what a partly finished install leaves is quarantine content and unreferenced package content, both of which are covered. Recorded rather than implied, so a later task that introduces a journal knows it owes a rule.
- [x] 2.11 Add security tests for archive parser boundaries, path normalization, signature substitution, revoked keys, stale witnesses, partial writes, and concurrent installs — the seven items on the original list that have machinery to test. Landed as `extension_platform::security_tests` and recorded in `design.md` under "The security suite, and what it found (Task 2.11)". The original wording said "fuzz targets"; what this delivers is a **deterministic mutation corpus** — every truncation and every single-byte corruption of a valid package and of a signature envelope, run through the real reader — and it is named that throughout. There is no continuous fuzzing harness and none is claimed: `cargo-fuzz` needs a nightly toolchain, a workspace member, and a CI job that do not exist, and adding them is its own change — task 15.5 already owns running a corpus long enough to produce retained evidence, and that is where a harness belongs if one is built. The corpus found three defects, all fixed: `PortablePackagePath` admitted Unicode bidirectional overrides; `create_owned_directory` reported a lost creation race as a filesystem error; and an archive appended to itself was read as one archive, which a signature cannot resolve because a publisher can sign a genuinely ambiguous file. The reader now refuses anything other than exactly one self-consistent end-of-central-directory record, and extraction consumes the inspected entry list so the two passes cannot disagree. The remaining two items on the original list — cancellation and version rollback — are tasks 4.10.1 and 4.10.2, because neither has machinery to test before Task Group 4.

## 3. Persistence and repositories

- [x] 3.1 Add additive SQLite migrations for publishers, packages, installations, snapshots, dependencies, contributions, runtime generations, operation witnesses, Hook definitions/bindings/executions, authorization rules/rule sets, connector definitions/instances/bindings. The trusted publisher-key table landed with task 2.4 as migration 83, the Developer Mode switch with task 2.5 as migration 84, and snapshots plus installations with task 2.9 as migration 85 — all because the required order puts those tasks first and each needed somewhere to write. Do not add a second table for any of them.
- [x] 3.2 Follow current migration numbering, transaction, foreign-key, timestamp, and repository conventions; do not add a second generic operations table.
- [x] 3.3 Add repositories with optimistic concurrency/version checks for installation state, active snapshot/generation pointers, Hook/rule generations, and connector state transitions.
- [x] 3.4 Add uniqueness constraints for every table this task group creates: extension id/version/hash, global contribution id, active generation, rule source/id, and connector source/id.
  `(extension_id, version, package_hash)` on snapshots plus the immutable version claim keyed on `(authority, extension_id, version)`; `(snapshot_id, global_id)` on contributions; one active-generation row per installation via its primary key, with the composite `(generation_id, installation_id)` reference that stops one installation pointing at another's; `(rule_set_id, source_kind, source_id, rule_id)` on rules; `(connector_global_id, label_key)` and `(instance_id, target_kind, target_key)` on connectors. The virtual Skill projection is **not** in this task's scope — its table is created by the contribution registry, and its constraint is task 6.3.1.
- [x] 3.5 Add retention and redaction bounds for Hook executions, runtime diagnostics, and operation witnesses; raw secrets and unrestricted environment data must be impossible to persist.
  No second `runtime_diagnostics` table. Hook executions reuse migration 87's bounds. Operation witnesses gain a `PersistableOperationWitness` the repository takes instead of a raw witness — one constructor, every bound checked: total canonical size, per-collection counts, per-field length, control characters refused rather than stripped, and a schema version (migration 90). Retention never removes evidence for an unfinished operation, a package that is active/rollback/quarantine, or a row a newer build wrote. `SafeExtensionRuntimeDiagnostic` is a host-owned code plus ids plus integer measurements — no string an extension chose can reach it, so redaction has nothing to miss — and it goes out through `platform::logging`. Real runtime producers arrive with Task Groups 4 and 5.
- [x] 3.6 Add the built-in seeding **mechanism** for extension and connector adapters -- idempotent, and incapable of overwriting user enablement, configuration, trust, or legacy integration state -- together with reviewed empty catalogs. The built-in *content* is not in this task's scope: host dispatch points are task 7.x, and the GitHub, IM, and MCP descriptors are tasks 10.7-10.9, each alongside the driver that gives the descriptor meaning.
  `seed_builtin_hooks` and `seed_builtin_connectors` take the subject and definition repositories and nothing else, so neither can create a binding, an instance, a credential, an enablement, or a configuration. Ownership is checked in both directions — a seed cannot take over an extension's subject and an extension cannot take a built-in id — with no `INSERT OR REPLACE` in either path. Same owner and same digest is idempotent; an upgrade is a new immutable revision under a new built-in snapshot, recorded beside the old one; the same snapshot claiming two digests is a stable conflict. Both catalogs are reviewed and empty in this build, and each says why in the code that returns it. Tests cover repeated launch, upgrade, both conflict directions, user-state preservation, partial-pass behaviour, and two concurrent seeds, driven by catalogs the tests supply -- so the mechanism is exercised without inventing content the later task groups have to decide.
- [x] 3.7 Add migration upgrade tests from current schema, repeated-start idempotency tests, rollback/compensation tests, and repository concurrency tests.
...
- [ ] 4.2.1 Close the witness-retention TOCTOU. `prune_operation_witnesses` takes the protection set as a parameter because storage cannot derive the unfinished-operation half of it, and a set computed before the prune can be stale by the time the delete commits: a package that became Active, a rollback target that was selected, a quarantine that was applied, or an operation that started in that window would have its evidence deleted by a caller that was right when it looked. Derive the database-knowable half -- active, previous, and quarantined package hashes -- *inside* the same write transaction rather than accepting it from the caller, and pin the caller-supplied unfinished-operation set to an operation-registry generation that the prune re-checks before committing. A stale protection set must never authorise a delete.
...
- [ ] 4.10 Add exhaustive state-machine tests, stale operation tests, concurrent enable/disable/reload tests, failure-injection tests at each lifecycle stage, and restart recovery tests.
  - [ ] 4.10.1 Cancel an in-flight install and prove it leaves no published content, no moved pointer, and no staged directory. Carried over from task 2.11, which had no operation or cancellation machinery to test against.
  - [ ] 4.10.2 Decide and enforce the version-rollback policy: whether installing a version older than the installed one is refused, or shown and confirmed. Carried over from task 2.11. The install witness already records the installed version alongside the offered one, so the downgrade is visible; what is missing is the decision and the rule that acts on it.
...
- [ ] 6.3.1 Add the uniqueness constraint for virtual Skill projection identity when this task group creates the projection table. Deferred from task 3.4, which added every other constraint in its list; there was no table to constrain, and Task Group 3 does not create one.
...
- [ ] 10.4.1 Add `connector_credential_transitions` as a durable state machine: `prepared` → `new_stored` → `switched` → `cleanup_pending`/`completed`, recording the predictable new handle **before** the secret is written. Migration 89 stores handles and defines `ConnectorCredentialPort`, and deliberately does **not** implement crash-safe replacement: `write-new → CAS → delete-old` survives an error return only, and a process killed between the credential-store write and the database commit leaves an orphaned secret and an instance pointing at the old handle. Task Group 3 has no connect or execute path, so there was no real replacement to make safe and no honest way to exercise recovery code.
- [ ] 10.4.2 Reconcile every non-terminal transition at startup, and add a test that kills the sequence at each state and asserts the next launch converges — no orphaned secret, no instance pointing at a handle that names nothing.
- [ ] 10.4.3 Assert no credential-store call happens inside a SQLite transaction.
- [ ] 10.4.4 Add a **scoped credential-use port** so a driver can borrow a secret for the duration of one call. `ConnectorCredentialPort` deliberately has no generic `read`: a method returning a secret is the one someone reaches for while building a DTO. The scoped port hands the secret to a closure, never returns it, and is the only way a driver ever sees one. A keychain read can block on a user prompt, a locked keyring, or a DBus round trip, and a write transaction holding the database lock while that happens stalls every other writer until the dialog is answered.
...
- [ ] 10.5.1 Re-validate and pin the Active Snapshot and Runtime Generation at the *start* of every side-effecting authenticate/test/connect/execute, and carry the pinned pair for the whole call. Task Group 3's reconciliation is stale-but-coherent by construction — it describes one whole generation, at worst one activation old — which is right for a report and not sufficient for a side effect. A disabled or rolled-back connector must not start a new call on a readiness verdict computed before the change.
...
- [ ] 10.11.1 Add tests that a connector disabled, rolled back, or whose definition became unavailable *between* the readiness check and the call is refused at the pin rather than dialled; and that a call already in flight keeps the generation it pinned instead of following the pointer.
