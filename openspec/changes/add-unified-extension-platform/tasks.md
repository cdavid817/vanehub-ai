...
- [x] 3.4 Add uniqueness constraints for extension id/version/hash, global contribution id, active generation, rule source/id, connector source/id, and virtual Skill projection identity.
  All but the last are in place: `(extension_id, version, package_hash)` on snapshots plus the immutable version claim keyed on `(authority, extension_id, version)`; `(snapshot_id, global_id)` on contributions; one active-generation row per installation via a primary key, with the composite `(generation_id, installation_id)` reference that stops one installation pointing at another's; `(rule_set_id, source_kind, source_id, rule_id)` on rules; and `(connector_global_id, label_key)` plus `(instance_id, target_kind, target_key)` on connectors. **The virtual Skill projection has no table yet** — it is created by the contribution registry in Task Group 6 — and a uniqueness constraint on a table that does not exist cannot be written. Recorded as task 6.3.1 rather than silently dropped.
...
- [x] 3.6 Seed built-in extension and connector adapters idempotently without overwriting user enablement, configuration, trust, or legacy integration state.
  `seed_builtin_hooks` and `seed_builtin_connectors` take the subject and definition repositories and nothing else, so neither can create a binding, an instance, a credential, an enablement, or a configuration. Ownership is checked in both directions — a seed cannot take over an extension's subject and an extension cannot take a built-in id — with no `INSERT OR REPLACE` in either path. Same owner and same digest is idempotent; an upgrade is a new immutable revision under a new built-in snapshot, recorded beside the old one; the same snapshot claiming two digests is a stable conflict. Both shipped catalogs are empty in this build and say so: host dispatch points arrive with Task Group 7, and GitHub/IM/MCP descriptors with the Task Group 10 tasks that bring their drivers. Tests cover repeated launch, upgrade, both conflict directions, user-state preservation, partial-pass behaviour, and two concurrent seeds.
