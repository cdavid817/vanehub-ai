...
- [ ] 10.4.3 Assert no credential-store call happens inside a SQLite transaction. A keychain read can block on a user prompt, a locked keyring, or a DBus round trip, and a write transaction holding the database lock while that happens stalls every other writer until the dialog is answered.
