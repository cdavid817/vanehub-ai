...
- [ ] 0.7 Extend the architecture fitness harness to cover the new capability. `scripts/architecture/frontend-rules.mjs` already rejects direct React `invoke()`, and `src-tauri/tests/architecture.rs` already rejects cross-context private-module and concrete-persistence imports, but its `source_scope` resolves `contexts/<context>/<layer>` only. `tooling` nests one level deeper, so every one of its nine subdomains — and the planned `permissions/rules` subdomain — is skipped rather than passing.
  ...
  - `contexts/tooling/extensions/application/ocr_admission.rs:2` imports `artifacts::application::{ArtifactDescriptor, ArtifactService}`. `artifacts` has no `api.rs`, which `openspec/project.md` already records as debt owed before a second consumer.
  - `contexts/tooling/skill_tools/domain/permission_manifest.rs:4` imports `std::net::IpAddr` for public-address classification across five sites; `std::net` is on the forbidden-technology list.

  Land the `source_scope` fix together with those repairs, in a change that owns `artifacts` and `skill_tools`. Do not add an architecture exception list to make this change pass.
