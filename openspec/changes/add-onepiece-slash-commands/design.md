...
改为 `appliesTo(session, capabilities)`，`capabilities` 由调度层组装后显式传入。测试无需置位与复位。

措辞需要精确：**双参数并不能让类型系统强制纯粹性**——TypeScript 的函数类型拦不住闭包捕获模块级变量，`appliesTo: (s) => someGlobalFlag` 照样编译通过。它真正做到的是把唯一已知的非 session 事实显式供上，从而**移除了伸手去拿全局变量的理由**。纯粹性因此是纪律，不是保证；写新命令时仍需自觉。
