...
- [ ] 10.4 Run dependency license/advisory review and targeted hostile-module fixtures on the supported Windows toolchain with the module feature both enabled and disabled.
  - License and advisory review passed, and Linux default/module-enabled hostile suites passed 138/143 tests. Windows remains NOT RUN: this host has no Windows runtime, Wine, MinGW, or Visual Studio toolchain. Installing the official `x86_64-pc-windows-msvc` Rust target allowed the cross-check to start, but `ring` correctly stopped at the missing MSVC `lib.exe`; this is not represented as a native Windows result. The repository's `windows-latest` CI runner can provide the required evidence once the current worktree is available to it.
