...
| `tooling` | CLI lifecycle and the MCP, SDK, extension, extension platform, plugin integration, Skill, Skill tool, and Prompt Hook subdomains |
...
- `cli_delegation` currently has no `api.rs`. It is consumed through the context that owns its calling behavior, so it has published no cross-context contract of its own. It MUST gain one before a second consumer depends on it, rather than that consumer reaching into its application modules. `artifacts` was in the same position and now publishes `api.rs`, which every consuming context and command imports.
