...
- `npm run test:desktop` 含六层,各自独立报结果与证据目录:`desktop-smoke`(启动、IPC、导航)、`desktop-cli-terminal`(CLI Agent 终端往返)、`desktop-session-workspace`(9 个工作区标签的真实内容)、`desktop-session-shell`(Shell 跨标签/会话切换存活,且只有显式确认的关闭才终止进程)、`desktop-dialogs`(主路径对话框的焦点与提交)、`desktop-settings-persistence`(设置改动跨真实重启)。可用 `npm run test:desktop:<层名>` 单独跑
