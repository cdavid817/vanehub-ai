...
- `npm run test:desktop` 含七层,各自独立报结果与证据目录:`desktop-smoke`(启动、IPC、导航)、`desktop-cli-terminal`(CLI Agent 终端往返)、`desktop-cli-management`(临时 PATH 上的 CLI 发现、计划、执行、验证与跨重启持久化)、`desktop-session-workspace`(9 个工作区标签的真实内容)、`desktop-dialogs`(主路径对话框的焦点与提交)、`desktop-scheduled-tasks`(定时任务的创建、启停与删除)、`desktop-settings-persistence`(设置改动跨真实重启)。可用 `npm run test:desktop:<层名>` 单独跑
