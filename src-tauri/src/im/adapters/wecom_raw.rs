...
impl RawWeComLongConnection {
    ...
    > {
        ...
        let (mut socket, _) = tokio_tungstenite::client_async_tls_with_config(&self.endpoint, stream, None, None)
            .await
...
impl WeComLongConnection for RawWeComLongConnection {
    ...
    async fn run(
        ...
        *self.outbound.lock().await = Some(sender);
        let result = self
            .run_session(bot_id, secret, &frames, &mut receiver, &mut shutdown, &mut ready)
            .await;
