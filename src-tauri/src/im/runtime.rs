use super::models::{
    ConnectorHealth, ConnectorKind, ConnectorLifecycle, NormalizedInbound, OutboundText,
};
...
const MAX_QUEUED_PER_CHAT: usize = 8;
...
fn classify_safe_code(code: &str) -> ConnectorErrorClass {
...
pub struct InboundDelivery {
    ...
    acceptance: oneshot::Sender<Result<(), ConnectorRuntimeError>>,
}
...
pub async fn submit_inbound(
    ...
    inbound
        .send(InboundDelivery { message, acceptance })
        .await
...
impl ImRuntimeManager {
    ...
    pub async fn health(&self) -> Vec<ConnectorHealth> {
        ...
        for (kind, managed) in connectors.iter() {
            ...
            result.push(ConnectorHealth {
                kind: *kind,
                lifecycle: state.lifecycle,
                generation: state.generation,
                safe_error_code: state.safe_error_code.clone(),
    ...
    pub async fn start(self: &Arc<Self>, kind: ConnectorKind) -> Result<(), ConnectorRuntimeError> {
        ...
        let generation = {
            ...
            state.generation += 1;
            state.lifecycle = ConnectorLifecycle::Connecting;
        ...
        let task = tokio::spawn(async move {
            ...
            match result {
                Ok(()) => {
                    state.lifecycle = ConnectorLifecycle::Disabled;
                    state.safe_error_code = None;
                    state.updated_at = Utc::now().to_rfc3339();
                }
                Err(error) => {
                    state.lifecycle = if error.class == ConnectorErrorClass::AuthorizationExpired {
                    ...
                    state.safe_error_code = Some(error.safe_code);
                    state.updated_at = Utc::now().to_rfc3339();
                }
            }
        });
    ...
    pub async fn stop(&self, kind: ConnectorKind) -> Result<(), ConnectorRuntimeError> {
        ...
        if let Some(mut task) = task {
            if tokio::time::timeout(STOP_TIMEOUT, &mut task).await.is_err() {
                task.abort();
                let mut state = managed.state.lock().await;
                state.lifecycle = ConnectorLifecycle::Error;
                state.safe_error_code = Some("shutdown-timeout".to_string());
                state.updated_at = Utc::now().to_rfc3339();
    ...
    async fn accept_inbound(
        ...
        if !inbound.direct || inbound.text.trim().is_empty() {
            self.diagnostic(
                inbound.connector,
                DiagnosticLevel::Debug,
                "ignore-inbound",
                if inbound.direct { "unsupported-content" } else { "group-message" },
                0,
            );
            return Ok(());
        }
        if !self.handler.claim(&inbound).await? {
            self.diagnostic(
                inbound.connector,
                DiagnosticLevel::Debug,
                "ignore-inbound",
        ...
        let reservation = QueueReservation { lane: Arc::clone(&lane) };
        let runtime = Arc::clone(self);
...
fn safe_platform_status_code(safe_code: &str) -> Option<String> {
    safe_code
        .rsplit('-')
        .next()
...
mod tests {
    ...
    async fn runs_all_five_connectors_through_dedup_binding_and_final_delivery() {
        ...
        let runtime = ImRuntimeManager::new(agent.clone());
        ...
        assert!(agent
            ...
            .unwrap()
            .iter()
            .any(|event| event.operation == "ignore-inbound" && event.safe_code == "duplicate-event"));
        assert!(runtime
            .health()
            .await
            .iter()
    ...
    async fn authentication_failure_enters_error_without_retrying() {
        ...
        let runtime = ImRuntimeManager::new(agent);
    ...
    async fn authorization_expiry_uses_dedicated_lifecycle_without_retrying() {
        ...
        tokio::time::timeout(Duration::from_secs(1), async {
            loop {
                if runtime.health().await[0].lifecycle
                    == ConnectorLifecycle::AuthorizationExpired
                {
                    break;
                }
    ...
    async fn ninth_pending_message_hits_per_chat_capacity_limit() {
        ...
        for index in 0..=MAX_QUEUED_PER_CHAT {
            runtime
