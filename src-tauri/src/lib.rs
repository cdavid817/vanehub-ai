...
fn load_chat_message_from_row(row: &Row<'_>) -> Result<ChatMessage, rusqlite::Error> {
    ...
    let file_references_json: Option<String> = row.get(11)?;
...
fn message_select_sql() -> &'static str {
    "SELECT id, session_id, role, status, content, thinking_content, tool_use, rich_blocks, token_input, token_output, metadata, file_references, created_at, updated_at FROM messages"
}
