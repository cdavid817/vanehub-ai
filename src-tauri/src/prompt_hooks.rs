...
pub(crate) fn list_prompt_hooks_for_conn(
    conn: &Connection,
...
fn builtin_to_hook(seed: &BuiltinPromptHook) -> PromptHook {
    ...
    PromptHook {
        ...
        cli_bindings: MANAGED_CLI_AGENT_IDS
            .iter()
            .map(|id| (*id).to_string())
            .collect(),
...
fn list_effective_hooks(conn: &Connection) -> Result<Vec<PromptHook>, AppError> {
    ...
    let mut hooks = BUILTIN_PROMPT_HOOKS
        ...
        .map(|hook| {
            overrides
                .get(&hook.id)
                .map_or(hook.clone(), |override_row| {
                    apply_override(hook, override_row)
                })
        })
        .collect::<Vec<_>>();
...
fn load_user_hooks(conn: &Connection) -> Result<Vec<PromptHook>, AppError> {
    ...
    let rows = stmt.query_map([], |row| {
        ...
        Ok(PromptHook {
            ...
            governance: serde_json::from_str(&governance_json)
                .unwrap_or_else(|_| default_governance(true)),
...
fn set_enabled(
    ...
    find_hook(conn, hook_id)?
        .ok_or_else(|| AppError::Validation(format!("Prompt Hook not found: {hook_id}")))
}
...
mod tests {
    ...
    fn user_hook_crud_and_binding_validation() {
        ...
        let rebound = set_cli_bindings(
            &mut conn,
            &created.id,
            vec!["codex-cli".to_string(), "codex-cli".to_string()],
        )
        .expect("bind");
    ...
    fn pipeline_skips_unbound_and_preserves_user_prompt() {
        ...
        .expect("bind");
        let result =
            assemble_prompt(&conn, "gemini-cli", Some("session-1"), "hello").expect("assemble");
        ...
        assert!(result
            .trace
            .iter()
            .all(|trace| trace.content_hash.as_deref() != Some("hello")));
    }
    ...
    fn pipeline_accepts_four_stable_cli_ids_and_rejects_unknown_agent() {
        ...
        for agent_id in ["claude-code", "codex-cli", "gemini-cli", "opencode"] {
            ...
            assert!(result
                .trace
                .iter()
                .any(|trace| trace.agent_id.as_deref() == Some(agent_id)));
        }
