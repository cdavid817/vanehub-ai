...
pub(crate) fn create_session_shell(
    ...
    api.create_session_shell(&request)
        .map(dto::descriptor_to_dto)
...
pub(crate) fn write_session_shell(
    ...
    api.write_session_shell(&request).map_err(map_command_error)
}
...
pub(crate) fn resize_session_shell(
    ...
    api.resize_session_shell(&request)
        .map_err(map_command_error)
}
...
#[tauri::command]
pub(crate) fn close_session_shell(
    api: State<'_, WorkspaceApi>,
    shell_id: String,
) -> Result<(), CommandError> {
    let shell_id = dto::shell_id(&shell_id).map_err(map_command_error)?;
    api.close_session_shell(&shell_id)
        .map_err(map_command_error)
}
