...
pub(crate) fn migrate(conn: &Connection) -> Result<(), DatabaseError> {
    ...
    // 87-89 rather than the 82-84 this change was written against: those numbers shipped in v1.1.0
    // and v1.2.0 carrying different schema, and a released version number is not available to be
    // reused. No installation has ever applied personalization at 82, so renumbering is additive.
    apply_transactional_migration(
        conn,
        87,
...
const EXPECTED_MIGRATIONS: &[(i64, &str)] = &[
    ...
    (87, "personalization-governance"),
    (88, "session-personalization-mode"),
    (89, "personalization-last-reconciled"),
];
