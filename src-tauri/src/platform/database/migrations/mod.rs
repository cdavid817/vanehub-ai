...
pub(crate) fn migrate(conn: &Connection) -> Result<(), DatabaseError> {
    ...
    // 112, moved up from 95 on this merge (redesign-unified-workbench-ui vs. origin/main): both
    // sides independently chose 95 for their own next migration while in review; 95-111 above
    // landed on main first, so this branch's own migration renumbers past them. Neither number has
    // reached an installation, so the renumber is additive.
    apply_migration(
        conn,
        112,
...
pub(super) const EXPECTED_MIGRATIONS: &[(i64, &str)] = &[
    ...
    // 112, moved up from 95 on this merge (redesign-unified-workbench-ui vs. origin/main): see the
    // matching comment next to the migration call above.
    (112, "scheduled-task-versioning"),
];
