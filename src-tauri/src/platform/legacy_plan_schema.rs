...
pub(crate) fn apply_retire_plan_execution_migration(
    connection: &Connection,
) -> Result<(), DatabaseError> {
    connection.execute_batch(
        r#"
        UPDATE plan_runs
        SET status = 'cancelled', driver_intent = 'stopped',
            completed_at = COALESCE(completed_at, updated_at)
        WHERE status NOT IN ('completed', 'failed', 'cancelled');

        INSERT INTO agent_run_events (run_id, sequence, state, timestamp, event_json)
        SELECT run_id,
               COALESCE((SELECT MAX(sequence) + 1 FROM agent_run_events AS event
                         WHERE event.run_id = agent_runs.run_id), 1),
               'cancelled',
               strftime('%Y-%m-%dT%H:%M:%fZ', 'now'),
               json_object(
                   'sequence', COALESCE((SELECT MAX(sequence) + 1
                                         FROM agent_run_events AS event
                                         WHERE event.run_id = agent_runs.run_id), 1),
                   'state', 'cancelled',
                   'trigger', 'cancel_shutdown',
                   'timestamp', strftime('%Y-%m-%dT%H:%M:%fZ', 'now'),
                   'reasonCode', 'plan_execution_retired',
                   'witness', 'migration:80'
               )
        FROM agent_runs
        WHERE owner_type = 'plan_run'
        ...
        SET state = 'cancelled',
            ...
            snapshot_json = json_insert(
                ...
                '$.events[#]',
                json((SELECT event_json FROM agent_run_events AS event
                      WHERE event.run_id = agent_runs.run_id
        ...
        WHERE id IN (
            SELECT retired.work_item_id
            FROM work_item_links AS retired
            WHERE retired.source_kind IN ('plan', 'plan_run')
              AND NOT EXISTS (
                  SELECT 1 FROM work_item_links AS retained
                  WHERE retained.work_item_id = retired.work_item_id
                    AND retained.source_kind NOT IN ('plan', 'plan_run')
              )
        );
        DELETE FROM work_item_links WHERE source_kind IN ('plan', 'plan_run');

        ALTER TABLE work_item_links RENAME TO legacy_work_item_links;
        ...
        INSERT INTO work_item_links (work_item_id, source_kind, source_id, relation, created_at)
        SELECT work_item_id, source_kind, source_id, relation, created_at
        FROM legacy_work_item_links;
        DROP TABLE legacy_work_item_links;
...
/// Replays the retired Plan schema so existing migration history remains valid without counting
/// compatibility-only DDL against the active database migration subtree's architecture budget.
///
...
/// Replays migration 62 after the runtime Plan context has been retired.
pub(crate) fn apply_legacy_plan_agent_loop_schema(
    connection: &Connection,
) -> Result<(), DatabaseError> {
    apply_legacy_plan_schema(connection)?;
    ...
    connection.execute_batch(
        ...
        CREATE TABLE IF NOT EXISTS plan_run_policies (
            ...
            max_attempts_per_subtask INTEGER NOT NULL CHECK (max_attempts_per_subtask BETWEEN 1 AND 5),
...
/// Replays migration 63 after the runtime Plan context has been retired.
pub(crate) fn apply_legacy_plan_session_association_schema(
    connection: &Connection,
) -> Result<(), DatabaseError> {
    apply_legacy_plan_agent_loop_schema(connection)?;
