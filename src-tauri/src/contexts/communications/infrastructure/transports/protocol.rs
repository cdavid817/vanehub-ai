...
use crate::contexts::communications::domain::{ConnectorKind, NormalizedInbound};
use serde_json::Value;

pub fn normalize_fixture(
    kind: ConnectorKind,
    payload: &str,
) -> Result<NormalizedInbound, ConnectorRuntimeError> {
    let value: Value = serde_json::from_str(payload)
        .map_err(|_| ConnectorRuntimeError::new("connector-payload-invalid"))?;
    match kind {
...
fn string_at(value: &Value, pointer: &str) -> Result<String, ConnectorRuntimeError> {
    value
        .pointer(pointer)
        .and_then(Value::as_str)
        .map(str::to_owned)
        .ok_or_else(|| ConnectorRuntimeError::new("connector-payload-field-missing"))
}

fn normalize_feishu(value: &Value) -> Result<NormalizedInbound, ConnectorRuntimeError> {
    let content = string_at(value, "/event/message/content")?;
    let content: Value = serde_json::from_str(&content)
        .map_err(|_| ConnectorRuntimeError::new("feishu-message-content-invalid"))?;
...
fn normalize_telegram(value: &Value) -> Result<NormalizedInbound, ConnectorRuntimeError> {
...
fn normalize_dingtalk(value: &Value) -> Result<NormalizedInbound, ConnectorRuntimeError> {
    ...
    Ok(NormalizedInbound {
        ...
        chat_id: if direct {
            ...
            string_at(value, "/data/openConversationId")?
        },
        sender_id,
...
fn normalize_wecom(value: &Value) -> Result<NormalizedInbound, ConnectorRuntimeError> {
    Ok(NormalizedInbound {
...
fn normalize_wechat(value: &Value) -> Result<NormalizedInbound, ConnectorRuntimeError> {
    Ok(NormalizedInbound {
