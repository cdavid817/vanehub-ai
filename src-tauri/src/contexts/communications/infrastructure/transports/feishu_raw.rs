...
impl RawFeishuLongConnection {
    async fn endpoint(
        ...
        app_id: &str,
        app_secret: &str,
    ) -> Result<EndpointData, ConnectorRuntimeError> {
        ...
        if !response.status().is_success() {
            return Err(ConnectorRuntimeError::new(format!(
                "feishu-ws-config-http-{}",
                response.status().as_u16()
            )));
        }
        let envelope = response
            .json::<EndpointEnvelope>()
            .await
            .map_err(|_| ConnectorRuntimeError::new("feishu-ws-config-invalid"))?;
        if envelope.code != 0 {
            return Err(ConnectorRuntimeError::new(format!(
                "feishu-ws-config-api-{}",
                envelope.code
            )));
        }
        envelope
            .data
            .ok_or_else(|| ConnectorRuntimeError::new("feishu-ws-config-missing"))
    }
    ...
    > {
        ...
        let socket =
            tokio_tungstenite::client_async_tls_with_config(endpoint.url, stream, None, None)
                .await
                .map(|(socket, _)| socket)
                .map_err(|_| ConnectorRuntimeError::new("feishu-ws-connect-failed"))?;
        Ok((
            socket,
            endpoint.client_config.ping_interval.max(10),
            service_id,
        ))
    }
    ...
    async fn accept_event(
        &self,
        frame: WireFrame,
    ) -> Result<Option<(String, String)>, ConnectorRuntimeError> {
        let headers = frame
            .headers
            .iter()
            .map(|header| (header.key.as_str(), header.value.as_str()))
            .collect::<HashMap<_, _>>();
        if headers.get("type").copied() != Some("event") {
            return Ok(None);
        }
        ...
        if message_id.is_empty() {
            return Err(ConnectorRuntimeError::new("feishu-ws-message-id-missing"));
        }
        let sum = headers
            .get("sum")
        ...
        let seq = headers
            ...
            .unwrap_or(0);
        if seq >= sum {
            return Err(ConnectorRuntimeError::new("feishu-ws-chunk-invalid"));
        }
        ...
        if assembly.parts.len() != sum {
            return Err(ConnectorRuntimeError::new("feishu-ws-chunk-count-changed"));
        }
        ...
        self.pending
            .lock()
            .await
            .insert(message_id.clone(), assembly.response_frame);
...
impl FeishuLongConnection for RawFeishuLongConnection {
    async fn test(&self, app_id: &str, app_secret: &str) -> Result<(), ConnectorRuntimeError> {
        ...
        socket
            .close(None)
            .await
            .map_err(|_| ConnectorRuntimeError::new("feishu-ws-test-close-failed"))
    }
    ...
    async fn acknowledge(&self, acknowledgement_id: &str) -> Result<(), ConnectorRuntimeError> {
        ...
        sender
            .send(frame)
            .await
            .map_err(|_| ConnectorRuntimeError::new("feishu-ws-ack-closed"))
    }
}
...
mod tests {
    ...
    async fn reassembles_event_chunks_before_ack_registration() {
        ...
        assert!(connection
            .accept_event(base(0, b"{\"ok\":"))
            .await
            .unwrap()
            .is_none());
        let merged = connection
            .accept_event(base(1, b"true}"))
            .await
            .unwrap()
            .unwrap();
