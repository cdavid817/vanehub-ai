    ...
    pub async fn poll_qr_status(
        ...
        Ok(match status {
            ...
            Some(Value::Number(value)) if value.as_i64() == Some(2) => {
                confirmed_credentials(&response.body)
            }
            ...
            Some(Value::String(value)) if value == "scanned" || value == "scaned" => {
                WeChatQrStatus::Scanned
            }
            Some(Value::String(value)) if value == "confirmed" => {
                confirmed_credentials(&response.body)
            }
...
mod tests {
    ...
    async fn accepts_official_scanned_and_confirmed_qr_states() {
        ...
        let WeChatQrStatus::Confirmed(credentials) =
            WeChatAdapter::poll_qr_status(http.as_ref(), "qr-private")
                .await
                .unwrap()
        else {
