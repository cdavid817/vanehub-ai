...
impl WorkspaceGitPort for FakeGit {
    ...
    fn validate_loop_worktree(
        ...
        self.validation_error.as_ref().map_or(Ok(()), |message| {
            Err(WorkspaceApplicationError::Validation(message.clone()))
        })
    }
    ...
    fn create_loop_worktree(
        ...
        self.creation_error.as_ref().map_or(Ok(()), |message| {
            Err(WorkspaceApplicationError::Validation(message.clone()))
        })
    }
}
...
fn service(calls: Arc<Mutex<Vec<String>>>) -> (WorkspaceApplicationService, FakeHistory) {
    service_with_git_errors(calls, None, None)
}

fn service_with_git_errors(
    calls: Arc<Mutex<Vec<String>>>,
    validation_error: Option<String>,
    creation_error: Option<String>,
) -> (WorkspaceApplicationService, FakeHistory) {
