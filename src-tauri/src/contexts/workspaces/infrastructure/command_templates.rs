use crate::contexts::workspaces::domain::{
    CommandTemplate, CommandTemplateError, CommandTemplateScope,
};
use crate::platform::database::NativeDatabase;
use rusqlite::{params, OptionalExtension};

#[derive(Clone)]
pub(crate) struct SqliteCommandTemplateRepository {
...
impl SqliteCommandTemplateRepository {
    ...
    pub(crate) fn save(&self, template: &CommandTemplate) -> Result<(), String> {
        template
            .validate()
            .map_err(|error: CommandTemplateError| error.to_string())?;
        let connection = self
            .database
            .connection()
    ...
    pub(crate) fn list(
        &self,
        scope: Option<CommandTemplateScope>,
        ...
        let rows = statement
            .query_map(
                params![scope.map(scope_name), connection_id, workspace_uri],
                read_template,
            )
            .map_err(|error| error.to_string())?;
    ...
    pub(crate) fn delete(&self, id: &str) -> Result<(), String> {
        ...
        connection
            .execute(
                "DELETE FROM terminal_command_templates WHERE id=?1",
                params![id],
            )
            .map_err(|error| error.to_string())?;
        Ok(())
...
fn scope_name(scope: CommandTemplateScope) -> &'static str {
    match scope {
        CommandTemplateScope::Global => "global",
        CommandTemplateScope::Connection => "connection",
        CommandTemplateScope::Workspace => "workspace",
    }
}
fn read_template(row: &rusqlite::Row<'_>) -> rusqlite::Result<CommandTemplate> {
    Ok(CommandTemplate {
        id: row.get(0)?,
        name: row.get(1)?,
        command: row.get(2)?,
        scope: match row.get::<_, String>(3)?.as_str() {
            "global" => CommandTemplateScope::Global,
            "connection" => CommandTemplateScope::Connection,
            _ => CommandTemplateScope::Workspace,
        },
        connection_id: row.get(4)?,
        workspace_uri: row.get(5)?,
