use crate::contexts::workspaces::application::{
    ShellLog, WorkspaceLogLevel, WorkspaceShellLogPort,
};

pub(crate) fn log_remote_terminal_event(
...
pub(crate) fn redact_remote_terminal_message(message: &str) -> String {
    ...
    for key in ["password", "token", "secret", "api_key", "private_key"] {
        ...
        if let Some(index) = lower.find(key) {
            if let Some(separator) = result[index..].find('=') {
                result.replace_range(index + separator + 1.., "[REDACTED]");
            }
        }
    }
    result
...
mod tests {
    ...
    fn redacts_sensitive_diagnostic_values() {
        let redacted = redact_remote_terminal_message(
            "password=hunter2 token=abc key_path=C:\\private\\id command=echo safe",
        );
