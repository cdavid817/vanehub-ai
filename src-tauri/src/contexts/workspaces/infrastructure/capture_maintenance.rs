...
#[derive(Clone)]
pub(crate) struct TerminalCaptureMaintenance {
...
impl TerminalCaptureMaintenance {
    ...
    pub(crate) fn purge_session(&self, session_id: &str) -> Result<usize, String> {
        ...
        let count = transaction
            .execute(
                "DELETE FROM terminal_output_chunks WHERE session_id=?1",
                params![session_id],
            )
            .map_err(|error| error.to_string())?;
    ...
    pub(crate) fn purge_before(&self, captured_before: &str) -> Result<usize, String> {
        ...
        let count = transaction
            .execute(
                "DELETE FROM terminal_output_chunks WHERE captured_at < ?1",
                params![captured_before],
            )
            .map_err(|error| error.to_string())?;
        transaction.commit().map_err(|error| error.to_string())?;
        Ok(count)
    ...
    pub(crate) fn enforce_capacity(&self, capacity_bytes: i64) -> Result<usize, String> {
        ...
        loop {
            let total: i64 = transaction
                .query_row(
                    "SELECT COALESCE(SUM(content_bytes),0) FROM terminal_output_chunks",
                    [],
                    |row| row.get(0),
                )
                .map_err(|error| error.to_string())?;
