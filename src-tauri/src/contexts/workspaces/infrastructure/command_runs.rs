...
#[derive(Clone)]
pub(crate) struct SqliteCommandRunRepository { database: NativeDatabase }
...
impl SqliteCommandRunRepository {
    ...
    pub(crate) fn list(&self, session_id: &str, offset: u32, limit: u32) -> Result<Vec<CommandRun>, String> {
        let connection = self.database.connection().map_err(|error| error.to_string())?;
        ...
        let rows = statement.query_map(params![session_id, limit.clamp(1, 100), offset], read_run).map_err(|error| error.to_string())?;
...
fn status_name(status: CommandRunStatus) -> &'static str { match status { CommandRunStatus::Queued => "queued", CommandRunStatus::Running => "running", CommandRunStatus::Succeeded => "succeeded", CommandRunStatus::Failed => "failed", CommandRunStatus::Cancelled => "cancelled" } }
fn read_run(row: &rusqlite::Row<'_>) -> rusqlite::Result<CommandRun> { Ok(CommandRun { id: row.get(0)?, template_id: row.get(1)?, session_id: row.get(2)?, connection_id: row.get(3)?, command_snapshot: row.get(4)?, working_directory: row.get(5)?, status: match row.get::<_, String>(6)?.as_str() { "queued" => CommandRunStatus::Queued, "running" => CommandRunStatus::Running, "succeeded" => CommandRunStatus::Succeeded, "cancelled" => CommandRunStatus::Cancelled, _ => CommandRunStatus::Failed }, exit_code: row.get(7)?, started_at: row.get(8)?, finished_at: row.get(9)? }) }
