...
pub(crate) use super::application::{PlanAttemptEvidenceView, PlanRunDetailView, PlanRunPageView};
use super::application::{PlanDiagnostic, PlanDiagnosticLevel, PlanDiagnosticsPort};
pub(crate) use super::domain::PlanDraft;
use super::infrastructure::{
    AttemptTerminalUpdate, NativeRecoveryEvidenceGateway, OnePieceAttemptExecutor,
    OnePieceAttemptVerifier, OnePiecePlanGenerator, PlanRunWorktree, SqlitePlanRepository,
};
...
impl TaskOrchestrationApi {
    pub(crate) fn native(
        ...
        Ok(Self {
            plans: PlanApplicationService::new(
                ...
                Arc::new(OnePiecePlanGenerator::new(agents)),
            ),
            repository,
            workspaces,
            executor,
    ...
    pub(crate) fn approve_plan(
        ...
        let run_id = self.plans.approve(plan_id, now)?;
        self.record_lifecycle(
            PlanDiagnosticLevel::Info,
            "plan.run.approved",
            Some(&run_id),
