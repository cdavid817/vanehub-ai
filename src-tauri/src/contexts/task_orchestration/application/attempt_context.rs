...
#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) struct AttemptRepairContext {
    pub(crate) attempt_sequence: u16,
    pub(crate) remaining_attempts: u16,
    pub(crate) error_class: String,
    pub(crate) failed_command_ids: Vec<String>,
    pub(crate) output_summaries: Vec<String>,
    pub(crate) changed_files: Vec<String>,
}
...
pub(crate) fn build_attempt_context(
    ...
    if let Some(repair) = &request.repair {
        prompt.push_str(&format!(
            ...
            repair.attempt_sequence,
            repair.remaining_attempts,
            repair.error_class,
            repair.failed_command_ids.join(", "),
            repair.changed_files.join(", ")
        ));
        for summary in &repair.output_summaries {
            prompt.push_str(&format!("- outputSummary: {summary}\n"));
        }
    }
...
mod tests {
    ...
    #[test]
    fn repair_context_contains_only_bounded_failure_evidence_and_budget() {
        let result = build_attempt_context(&AttemptContextRequest {
            plan_run_id: "run-1".into(),
            subtask_run_id: "task-run-1".into(),
            task: task(),
            direct_predecessor_ids: Vec::new(),
            predecessor_sources: Vec::new(),
            repair: Some(AttemptRepairContext {
                attempt_sequence: 2,
                remaining_attempts: 1,
                error_class: "verification_failed".into(),
                failed_command_ids: vec!["tests".into()],
                output_summaries: vec!["bounded assertion failure".into()],
                changed_files: vec!["src/lib.rs".into()],
            }),
            character_budget: 4_000,
        })
        .expect("repair context");
        assert!(result.prompt.contains("attemptSequence: 2"));
        assert!(result.prompt.contains("remainingAttempts: 1"));
        assert!(result.prompt.contains("failedCommands: tests"));
        assert!(result.prompt.contains("bounded assertion failure"));
        assert!(!result.prompt.contains("rawTranscript"));
    }
}
