...
use crate::contexts::task_orchestration::domain::{
    validate_plan_execution_policy, CriterionEvidenceBinding, DependencyEdge,
    PlanDiscoveryMetadata, PlanDraft, PlanExecutionPolicy, ResourceLimits, SubTaskSpec,
    VerificationCommand,
};
...
pub(crate) fn parse_planner_response(
    ...
    let mut subtasks = Vec::with_capacity(proposed_subtasks.len());
    for (ordinal, proposal) in proposed_subtasks.into_iter().enumerate() {
        ...
        subtasks.push(SubTaskSpec {
            ...
            criterion_evidence: proposal.criterion_evidence,
    ...
    let dependencies = proposed_dependencies
        .into_iter()
        .map(|edge| {
...
mod tests {
        ...
        let response = PlanGenerationResponse {
            ...
            content: r#"{
                ...
                "executionPolicy":{
                    ...
                    "finalValidationCommands":[{"id":"final","program":"cargo","args":["test"],"workingDirectory":null,"timeoutSeconds":300,"required":true}]
                },
                "subtasks": [
                    {"id":"a","title":"Design","description":"Design it","acceptanceCriteria":["Design accepted"],"criterionEvidence":[{"criterionIndex":0,"kind":"automated","commandId":"check-a"}],"validationCommands":[{"id":"check-a","program":"cargo","args":["check"],"workingDirectory":null,"timeoutSeconds":300,"required":true}]},
                    {"id":"b","title":"Build","description":"Build it","acceptanceCriteria":["Tests pass"],"criterionEvidence":[{"criterionIndex":0,"kind":"automated","commandId":"test-b"}],"validationCommands":[{"id":"test-b","program":"cargo","args":["test"],"workingDirectory":null,"timeoutSeconds":300,"required":true}]}
                ],
    ...
    #[test]
    fn parses_limited_discovery_and_rejects_invalid_evidence_or_final_commands() {
        let limited = PlanGenerationResponse {
            active_profile_id: "profile-1".into(),
            content: r#"{
                "discovery":{"status":"limited","limitations":["entry budget reached"]},
                "executionPolicy":{"maxAttemptsPerSubtask":2,"repairEligibleClasses":["verification_failed"],"finalValidationCommands":[{"id":"final","program":"cargo","args":["test"],"workingDirectory":null,"timeoutSeconds":300,"required":true}]},
                "subtasks":[{"id":"a","title":"A","description":"A","acceptanceCriteria":["ok"],"criterionEvidence":[{"criterionIndex":0,"kind":"automated","commandId":"test"}],"validationCommands":[{"id":"test","program":"cargo","args":["test"],"workingDirectory":null,"timeoutSeconds":300,"required":true}]}],
                "dependencies":[]
            }"#.into(),
        };
        let draft = parse_planner_response(&request(), limited).expect("limited draft");
        assert_eq!(
            draft.discovery.status,
            crate::contexts::task_orchestration::domain::PlanDiscoveryStatus::Limited
        );

        let invalid_binding = PlanGenerationResponse {
            active_profile_id: "profile-1".into(),
            content: r#"{
                "discovery":{"status":"complete","limitations":[]},
                "executionPolicy":{"maxAttemptsPerSubtask":2,"repairEligibleClasses":["verification_failed"],"finalValidationCommands":[{"id":"final","program":"cargo","args":["test"],"workingDirectory":null,"timeoutSeconds":300,"required":true}]},
                "subtasks":[{"id":"a","title":"A","description":"A","acceptanceCriteria":["ok"],"criterionEvidence":[{"criterionIndex":0,"kind":"automated","commandId":"missing"}],"validationCommands":[{"id":"test","program":"cargo","args":["test"],"workingDirectory":null,"timeoutSeconds":300,"required":true}]}],
                "dependencies":[]
            }"#.into(),
        };
        assert!(parse_planner_response(&request(), invalid_binding).is_err());

        let malformed_final = PlanGenerationResponse {
            active_profile_id: "profile-1".into(),
            content: r#"{"discovery":{"status":"complete","limitations":[]},"executionPolicy":{"maxAttemptsPerSubtask":2,"repairEligibleClasses":["verification_failed"],"finalValidationCommands":[{"id":"final","program":"","args":[],"workingDirectory":null,"timeoutSeconds":0,"required":true}]},"subtasks":[],"dependencies":[]}"#.into(),
        };
        assert!(parse_planner_response(&request(), malformed_final).is_err());
    }
}
