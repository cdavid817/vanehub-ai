...
use crate::contexts::task_orchestration::application::{
    PlanApplicationError, PlanAttemptEvidenceView, PlanFinalRepairView, PlanFinalizationView,
    PlanRunDetailView, PlanRunPageView, PlanRunSummaryView, PlanSubTaskAttemptView,
    PlanSubTaskRunView,
};
...
impl SqlitePlanRepository {
    ...
    pub(crate) fn get_run_detail(
        ...
        let finalization = read_finalization(&connection, run_id)?;
        let can_retry_final = can_retry_final_verification(&connection, run_id)?;
        let available_controls = controls_for_status(&header.0.status, can_retry_final);
        Ok(PlanRunDetailView {
            ...
            originating_session_id: header.7,
            tasks,
            finalization,
            available_controls,
        })
    ...
    pub(crate) fn find_run_for_originating_session(
        &self,
        session_id: &str,
        ...
        connection
            .query_row(
                &format!(
                    "{} WHERE run.originating_session_id = ?1 GROUP BY run.id ORDER BY run.created_at DESC, run.id DESC LIMIT 1",
                    summary_select()
                ),
                [session_id],
                read_summary,
...
fn read_finalization(
    ...
    run_id: &str,
) -> Result<Option<PlanFinalizationView>, PlanApplicationError> {
    let header = connection
        .query_row(
            r#"SELECT id, sequence, status FROM plan_finalizations
               WHERE plan_run_id = ?1 ORDER BY sequence DESC LIMIT 1"#,
            [run_id],
            |row| {
                Ok((
                    row.get::<_, String>(0)?,
        ...
        .optional()
        .map_err(storage_error)?;
    let Some((id, sequence, status)) = header else {
        return Ok(None);
    };
    let mut statement = connection
        .prepare(
            r#"SELECT id, command_id, status, exit_code, duration_ms, output_summary, created_at
               FROM plan_final_verification_evidence WHERE finalization_id = ?1
               ORDER BY created_at, id"#,
        )
        .map_err(storage_error)?;
    let evidence = statement
        .query_map([&id], read_evidence_row)
        .map_err(storage_error)?
        .collect::<Result<Vec<_>, _>>()
        .map_err(storage_error)?;
    let mut repair_statement = connection
        .prepare(
            r#"SELECT id, sequence, status, session_id, token_usage, tool_call_count,
                      error_class, started_at, completed_at
               FROM plan_final_repair_attempts WHERE finalization_id = ?1
               ORDER BY sequence, started_at, id"#,
        )
        .map_err(storage_error)?;
    let repair_attempts = repair_statement
        .query_map([&id], |row| {
            Ok(PlanFinalRepairView {
                id: row.get(0)?,
                sequence: row.get(1)?,
                status: row.get(2)?,
...
fn detail_select() -> &'static str {
    r#"SELECT run.id, run.plan_id, run.status,
              ...
              run.worktree_name, run.worktree_branch, run.originating_session_id
       FROM plan_runs AS run
       LEFT JOIN plan_subtask_runs AS task ON task.plan_run_id = run.id"#
}
...
fn read_evidence(
    ...
    let mut statement = connection
        ...
        .map_err(storage_error)?;
    let result = statement
        .query_map([attempt_id], read_evidence_row)
        .map_err(storage_error)?
        .collect::<Result<Vec<_>, _>>()
        .map_err(storage_error);
    result
...
fn read_evidence_row(row: &Row<'_>) -> rusqlite::Result<PlanAttemptEvidenceView> {
...
fn can_retry_final_verification(
    connection: &crate::platform::database::PooledSqlite,
    run_id: &str,
    ...
    connection
        .query_row(
            r#"SELECT COUNT(finalization.id) < policy.max_attempts_per_subtask
               ...
               GROUP BY run.id, policy.max_attempts_per_subtask"#,
            [run_id],
            |row| row.get(0),
        )
        .optional()
...
fn controls_for_status(status: &str, can_retry_final: bool) -> Vec<String> {
    match PlanRunStatus::parse(status) {
        Some(PlanRunStatus::Running | PlanRunStatus::Verifying | PlanRunStatus::Repairing) => {
        ...
        Some(PlanRunStatus::ActionRequired) if can_retry_final => {
            vec!["retry".into(), "cancel".into()]
        }
        Some(PlanRunStatus::ActionRequired) => vec!["cancel".into()],
        Some(PlanRunStatus::FinalVerifying) => vec!["cancel".into()],
