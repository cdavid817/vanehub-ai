...
impl OnePieceAttemptExecutor {
    ...
    fn classify_terminal_failure(
        ...
        let diagnostic = self
            ...
            .unwrap_or_default();
        classify_failure_diagnostic(outcome, &diagnostic).to_string()
    }
}

fn classify_failure_diagnostic(
    outcome: AgentMessageTerminalOutcome,
    diagnostic: &str,
) -> &'static str {
    if outcome == AgentMessageTerminalOutcome::Cancelled {
        "cancelled"
    } else if diagnostic.contains("orchestration timeout") {
        "timeout"
    } else if diagnostic.contains("orchestration tool-call limit") {
        "tool_call_limit"
    } else if diagnostic.contains("orchestration token") {
        "token_budget"
    } else if diagnostic.contains("credential") || diagnostic.contains("API key") {
        "missing_credentials"
    } else if diagnostic.contains("safety") || diagnostic.contains("permission denied") {
        "safety_rejected"
    } else {
        "provider_failed"
    }
...
mod tests {
    ...
    #[test]
    fn terminal_failures_have_stable_repair_safety_classes() {
        let failed = AgentMessageTerminalOutcome::Failed;
        assert_eq!(
            classify_failure_diagnostic(failed, "orchestration timeout reached"),
            "timeout"
        );
        assert_eq!(
            classify_failure_diagnostic(failed, "orchestration tool-call limit reached"),
            "tool_call_limit"
        );
        assert_eq!(
            classify_failure_diagnostic(failed, "orchestration token budget reached"),
            "token_budget"
        );
        assert_eq!(
            classify_failure_diagnostic(failed, "provider credential is missing"),
            "missing_credentials"
        );
        assert_eq!(
            classify_failure_diagnostic(failed, "permission denied by safety policy"),
            "safety_rejected"
        );
        assert_eq!(
            classify_failure_diagnostic(AgentMessageTerminalOutcome::Cancelled, ""),
            "cancelled"
        );
        assert_eq!(
            classify_failure_diagnostic(failed, "unexpected provider response"),
            "provider_failed"
        );
    }
}
