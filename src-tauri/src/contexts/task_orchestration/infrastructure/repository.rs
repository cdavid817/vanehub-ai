...
use crate::contexts::task_orchestration::domain::{
    validate_plan_graph, DependencyEdge, PlanDraft, PlanRunStatus, PlanStatus, ResourceLimits,
...
impl SqlitePlanRepository {
    ...
    pub(crate) fn schedule_next(
        ...
        if let Some(next_id) = &decision.next_id {
            ...
            let next = match decision.projection {
                RunProjection::AwaitingAcceptance => PlanRunStatus::AwaitingAcceptance,
                RunProjection::Failed => PlanRunStatus::Failed,
    ...
    pub(crate) fn project_after_attempt(
        ...
        let terminal = match decision.projection {
            RunProjection::AwaitingAcceptance => Some(PlanRunStatus::AwaitingAcceptance),
            RunProjection::Failed => Some(PlanRunStatus::Failed),
...
impl PlanRepositoryPort for SqlitePlanRepository {
    ...
    fn approve_latest(&self, plan_id: &str, now: &str) -> Result<String, PlanApplicationError> {
        SqlitePlanRepository::approve_latest(self, plan_id, now)
    }
}
    ...
    transaction
        .execute(
            r#"INSERT INTO plan_versions (
                   id, plan_id, version, goal, project_path, base_ref, planner_profile_id, created_at
...
mod tests {
    ...
    #[test]
    fn final_verified_attempt_immediately_projects_awaiting_acceptance() {
        let repository = repository();
        repository.save_draft(&draft()).expect("save");
        let run_id = repository
            .approve_latest("plan-1", "2026-08-08T00:00:00Z")
        ...
        let status: String = repository
            ...
            .expect("status");
        assert_eq!(status, "awaiting_acceptance");
    }
