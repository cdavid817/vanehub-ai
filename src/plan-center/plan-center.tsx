...
export function PlanCenter({ onRunAssociated, originatingSessionId = null, requestedRunId = null }: { onRunAssociated?: (runId: string) => void; originatingSessionId?: string | null; requestedRunId?: string | null }) {
  ...
  useEffect(() => {
    let cancelled = false;
    if (!requestedRunId || requestedRunId === runId) return () => { cancelled = true; };
    setBusy(true);
    void planService.getPlanRun(requestedRunId)
      .then((result) => { if (!cancelled) { setRun(result); setError(null); } })
  ...
  useEffect(() => {
    if (!runId || !runStatus || ["completed", "cancelled", "failed", "action_required", "awaiting_acceptance"].includes(runStatus)) return;
  ...
  const approve = () => perform(async () => {
    ...
    const approved = await planService.approvePlan(draft.id, originatingSessionId ?? undefined);
    await planService.startPlanRun(approved.runId);
    setRun(await planService.getPlanRun(approved.runId));
    if (originatingSessionId) onRunAssociated?.(approved.runId);
  });
  ...
  const returnToPlanning = () => {
    if (!draft) return;
    setDraft({ ...structuredClone(draft), version: draft.version + 1, versionId: crypto.randomUUID() });
    setRun(null);
  };
  ...
  return (
    <section aria-labelledby="plan-center-title" className="ucd-panel flex h-full min-h-0 flex-1 flex-col overflow-hidden rounded-lg">
      ...
      <div aria-atomic="true" aria-live="polite" className="min-h-0 flex-1 overflow-y-auto p-3 md:p-5">
        ...
        {run ? <PlanRunView busy={busy} onAccept={() => void perform(async () => setRun((await planService.acceptPlanRun(run.id)).run))} onControl={(kind) => void control(kind)} onInspectEvidence={(attemptId) => planService.getPlanAttemptEvidence(attemptId)} onRetry={(taskId) => void perform(async () => setRun((await planService.retryPlanSubtask(run.id, taskId)).run))} onReturnPlanning={returnToPlanning} run={run} /> : draft ? <div className="mx-auto grid max-w-5xl gap-4"><PlanDraftEditor draft={draft} onChange={setDraft} /><div className="rounded-lg border border-primary/30 bg-primary/5 p-4" role="status"><h3 className="text-sm font-semibold">{t("plans.review.approvalSummary")}</h3><p className="mt-1 text-xs text-muted-foreground">{t("plans.review.approvalScope", { project: draft.projectPath, tasks: draft.subtasks.length, commands: draft.subtasks.reduce((count, task) => count + task.validationCommands.filter((command) => command.required).length, draft.executionPolicy.finalValidationCommands.filter((command) => command.required).length) })}</p><p className="mt-1 text-xs text-muted-foreground">{t("plans.review.retainedWorktree")}</p></div><div className="sticky bottom-0 flex justify-end gap-2 border-t border-border bg-background/95 py-3 backdrop-blur"><Button disabled={busy} onClick={() => setDraft(null)} type="button" variant="outline">{t("plans.review.back")}</Button><Button disabled={busy} onClick={() => void approve()} type="button">{busy ? <Loader2 aria-hidden="true" className="animate-spin" /> : <Sparkles aria-hidden="true" />}{t("plans.review.approve")}</Button></div></div> : <GoalForm baseRef={baseRef} busy={busy} goal={goal} onBaseRef={setBaseRef} onGenerate={() => void generate()} onGoal={setGoal} onProjectPath={setProjectPath} projectPath={projectPath} />}
      </div>
    </section>
  );
}
