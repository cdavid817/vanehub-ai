...
export function PlanDraftEditor({ draft, onChange }: { draft: PlanDraft; onChange: (draft: PlanDraft) => void }) {
  ...
  const addTask = () => {
    ...
    onChange({ ...draft, subtasks: [...draft.subtasks, {
      ...
      limits: { tokenBudget: 8_000, toolCallLimit: 30, timeoutSeconds: 900 }, validationCommands: [],
    }] });
  };
      ...
      <ol className="grid gap-3">
        {draft.subtasks.map((task, index) => (
          <li className="ucd-card grid gap-3 rounded-lg p-3" key={task.id}>
            ...
            <label className="grid gap-1 text-xs font-medium text-muted-foreground">{t("plans.review.criteria")}<textarea className={`${inputClass} min-h-20`} onChange={(event) => updateTask(task.id, { acceptanceCriteria: event.target.value.split("\n").map((value) => value.trim()).filter(Boolean).slice(0, 3) })} value={task.acceptanceCriteria.join("\n")} /></label>
