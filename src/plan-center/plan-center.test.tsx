...
const draft: PlanDraft = {
  ...
  baseRef: "main", plannerProfileId: "profile-1", dependencies: [], subtasks: [{
    criterionEvidence: [{ criterionIndex: 0, kind: "automated", commandId: "tests" }],
    ...
    validationCommands: [{ id: "tests", program: "npm", args: ["test"], workingDirectory: null, timeoutSeconds: 60, required: true }],
  }],
...
describe("Plan Center", () => {
  ...
  it("collects a goal and enters editable human review after OnePiece generation", async () => {
    ...
    expect(screen.getByLabelText("Maximum attempts per task")).toBeTruthy();
    expect(screen.getAllByLabelText("Command ID").length).toBeGreaterThan(1);
    expect(screen.getByLabelText("Evidence type for criterion 1")).toBeTruthy();
  ...
  it("shows verified progress, explicit acceptance, and retained-worktree safety copy", async () => {
    const accept = vi.fn();
    render(<PlanRunView busy={false} onAccept={accept} onControl={vi.fn()} onInspectEvidence={service.getPlanAttemptEvidence} onRetry={vi.fn()} run={run} />);
  ...
  it("routes controls and loads bounded attempt evidence only after explicit inspection", async () => {
    ...
    const view = render(<PlanRunView busy={false} onAccept={vi.fn()} onControl={control} onInspectEvidence={service.getPlanAttemptEvidence} onRetry={retry} run={active} />);
    ...
    view.rerender(<PlanRunView busy={false} onAccept={vi.fn()} onControl={control} onInspectEvidence={service.getPlanAttemptEvidence} onRetry={retry} run={{ ...active, status: "recovery_required", availableControls: ["recover"] }} />);
    fireEvent.click(screen.getByRole("button", { name: "Recover" }));
    view.rerender(<PlanRunView busy={false} onAccept={vi.fn()} onControl={control} onInspectEvidence={service.getPlanAttemptEvidence} onRetry={retry} run={{ ...active, status: "paused", availableControls: ["resume"] }} />);
    fireEvent.click(screen.getByRole("button", { name: "Resume" }));
    view.rerender(<PlanRunView busy={false} onAccept={vi.fn()} onControl={control} onInspectEvidence={service.getPlanAttemptEvidence} onRetry={retry} run={{ ...active, status: "action_required", availableControls: ["retry", "cancel"], finalization: { id: "final-1", sequence: 1, status: "failed", evidence: [{ id: "final-evidence", commandId: "final-tests", status: "failed", exitCode: 1, durationMs: 20, outputSummary: "final check failed", createdAt: "2026-08-08T00:00:20Z" }], repairAttempts: [{ id: "repair-1", sequence: 1, status: "failed", sessionId: "repair-session", tokenUsage: 20, toolCallCount: 2, errorClass: "provider_failed", startedAt: "2026-08-08T00:00:21Z", completedAt: "2026-08-08T00:00:22Z" }] } }} />);
    fireEvent.click(screen.getByRole("button", { name: "Retry final checks" }));
    expect(screen.getByText("final check failed")).toBeTruthy();
    expect(screen.getByText(/Repair 1.*failed/)).toBeTruthy();
    expect(control.mock.calls).toEqual([["pause"], ["cancel"], ["recover"], ["resume"], ["retry"]]);
    expect(retry).toHaveBeenCalledWith("task-run-1");
    expect(screen.getByText(/session-1/)).toBeTruthy();
  });

  it("returns an action-required run to a new editable Plan version", async () => {
    service.approvePlan.mockResolvedValue({ runId: run.id, summary: { projectPath: draft.projectPath, taskCount: 1, retainedWorktree: true, automaticGitOperations: false } });
    service.startPlanRun.mockResolvedValue({ runId: run.id, status: "running", projectPath: draft.projectPath, baseOid: "abc", worktreePath: "D:/app-plan", worktreeName: "plan", worktreeBranch: "vanehub/plan" });
    service.getPlanRun.mockResolvedValue({ ...run, status: "action_required", availableControls: ["cancel"] });
    render(<PlanCenter />);
    fireEvent.change(screen.getByRole("textbox", { name: "Goal" }), { target: { value: "Ship safely" } });
    fireEvent.change(screen.getByRole("textbox", { name: "Project path" }), { target: { value: "D:/app" } });
    fireEvent.click(screen.getByRole("button", { name: "Generate Plan" }));
    await screen.findByText("Review task graph");
    fireEvent.click(screen.getByRole("button", { name: "Approve and start" }));
    fireEvent.click(await screen.findByRole("button", { name: "Return to planning" }));
    expect(await screen.findByText("Review task graph")).toBeTruthy();
    fireEvent.click(screen.getByRole("button", { name: "Approve and start" }));
    await waitFor(() => expect(service.savePlanDraft).toHaveBeenLastCalledWith(expect.objectContaining({ id: draft.id, version: 2 })));
    const revised = service.savePlanDraft.mock.calls.at(-1)?.[0] as PlanDraft;
    expect(revised.versionId).not.toBe(draft.versionId);
  });
});
