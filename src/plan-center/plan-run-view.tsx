import { Check, CircleStop, Pause, Play, RefreshCw, RotateCcw } from "lucide-react";
...
export function PlanRunView({ busy, onAccept, onControl, onExecute, onInspectEvidence, onRetry, run }: {
  busy: boolean;
  onAccept: () => void;
      ...
      <header className="ucd-card grid gap-3 rounded-lg p-4">
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex flex-wrap gap-2">
            {run.status === "running" ? <Button disabled={busy} onClick={onExecute} size="sm" type="button"><Play aria-hidden="true" />{t("plans.run.executeNext")}</Button> : null}
