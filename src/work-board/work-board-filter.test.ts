...
describe("work board filters", () => {
  it("combines text, source, project, stage, priority, and archive filters", () => {
    const multiSource = item({ sources: [
      ...
      { sourceKind: "plan", sourceId: "p", relation: "primary", title: "Plan", status: "approved", available: true, projectPath: "D:/app", updatedAt: null },
    ] });
    expect(filterWorkItems([multiSource], { query: "build", sourceKinds: ["plan"], projectPaths: ["D:/app"], stages: ["review"], priorities: ["high"] })).toEqual([multiSource]);
    expect(filterWorkItems([multiSource], { sourceKinds: ["scheduled_task"] })).toEqual([]);
