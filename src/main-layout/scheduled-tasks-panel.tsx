...
/**
 * The content half of what used to be `ScheduledTasksDialog` — extracted so it can render as a
 * routed `/runs/schedules` page (design.md task 4.6) instead of only inside a modal. State reset
 * and the initial task load used to depend on the dialog's `open` prop; here they run once on
 * mount instead, since a routed page is only ever rendered while "open."
 */
export function ScheduledTasksPanel({ agents }: { agents: AgentRegistryEntry[] }) {
