...
import type { KnownProject, ProjectInspection } from "../types/agent";
      ...
      <div className="grid grid-cols-2 gap-2">
        {(["local", "remote"] as const).map((candidate) => (
          ...
          >
            ...
            {candidate === "local"
              ? t("createSession.workspaceMode.local")
              : t("createSession.workspaceMode.remote")}
          </button>
        ))}
      </div>
    </section>
      ...
      <section className="grid gap-2">
        ...
        {knownProjects.length > 0 ? (
          <div className="grid gap-1">
            <p className="px-1 text-xs font-medium text-muted-foreground">
              {t("createSession.recentProjects")}
            </p>
            {knownProjects.slice(0, 5).map((project) => (
              ...
              >
                ...
                <span className="min-w-0 flex-1 truncate">
                  {normalizeDisplayPath(project.path)}
                </span>
                <span className="text-muted-foreground">
                  {project.isGit
                    ? t("createSession.folderType.git")
                    : t("createSession.folderType.folder")}
                </span>
              </button>
            ))}
          </div>
        ...
        {inspection ? (
          <p className="text-xs text-muted-foreground">
            {inspection.isGit
              ? t("createSession.gitProject")
              : t("createSession.normalFolder")}
          </p>
        ) : null}
      </section>
      ...
      <section className="ucd-muted-panel grid gap-2 rounded-md p-3">
        <label
          className={cn(
            ...
            !gitCapable && "text-muted-foreground",
          )}
        >
          <input
