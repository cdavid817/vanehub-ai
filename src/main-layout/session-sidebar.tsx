...
import { Archive, CheckSquare, ChevronDown, ChevronRight, List, ListTree, Pin, Plus, Search, Trash2, X } from "lucide-react";
  ...
  return (
    ...
    >
      ...
      <div className="flex min-w-0 items-center gap-2">
        {batchMode ? (
          <input
            aria-label={t("layout.batchSelectSession")}
            checked={checked}
...
export function SessionSidebar({ activeSessionId, agentsAvailable, archivedSessions, categories, deletingSessions, onAssignCategory, onBatchDelete, onContextMenu, onNew, onSearchChange, onSelect, searchQuery, searchResults, sessions }: {
  ...
  const [presentation, setPresentation] = useState<SessionPresentationMode>("list");
  ...
  const categoryGroups = useMemo(() => {
    const pool = sourceMode === "active" && !searchQuery.trim() ? filteredSessions.filter((session) => !session.pinned) : renderedSessions;
    return [
      ...categories.map((category) => ({ id: category.id, label: category.name, sessions: pool.filter((session) => session.categoryId === category.id) })),
      { id: null, label: t("layout.uncategorized"), sessions: pool.filter((session) => !session.categoryId) },
    ];
  }, [categories, filteredSessions, renderedSessions, searchQuery, sourceMode, t]);
  ...
  return (
    <aside className="ucd-panel flex h-full min-h-0 w-full flex-col rounded-lg p-3 max-[640px]:max-h-64" onContextMenu={(event) => event.preventDefault()}>
      ...
      <div className="mb-2 grid grid-cols-2 gap-1">
        <Button className="h-7 px-2 text-xs" onClick={() => setBatchMode(true)} size="sm" variant={batchMode ? "default" : "outline"}><CheckSquare aria-hidden="true" className="h-3.5 w-3.5" />{batchMode ? t("layout.batchManaging") : t("layout.batchManage")}</Button>
        <Button className="h-7 px-2 text-xs" onClick={() => setSourceMode((mode) => mode === "archived" ? "active" : "archived")} size="sm" variant={sourceMode === "archived" ? "default" : "outline"}><Archive aria-hidden="true" className="h-3.5 w-3.5" />{sourceMode === "archived" ? t("layout.archived") : `${t("layout.archive")} ${archivedSessions.length}`}</Button>
      </div>
      <div className="ucd-segmented mb-2 grid grid-cols-2 gap-1 rounded-md p-1">
      ...
      <select aria-label={t("layout.agentFilter")} className="ucd-input mb-2 h-8 rounded-md px-2 text-xs" onChange={(event) => setAgentFilter(event.target.value as SessionAgentFilter)} value={agentFilter}>
        {sessionAgentFilters.map((filter) => <option key={filter} value={filter}>{t(`layout.agentFilter.${filter}`)}</option>)}
      </select>
      {batchMode ? (
        <div className="ucd-muted-panel mb-2 grid gap-2 rounded-md p-2">
