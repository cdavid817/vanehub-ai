...
export function SessionContextPanel({ categories, onArchive, onAssignCategory, onChange, onCreateCategory, onDelete, onDismiss, onExport, onPin, onRecover, onRename, recovering = false, value }: {
  ...
  return <div className="fixed inset-0 z-50 grid place-items-center bg-background/60 p-4"><div className="ucd-panel grid w-full max-w-sm gap-3 rounded-lg p-4 text-sm shadow-xl"><div><h3 className="font-semibold">{t("layout.deleteSession")}</h3><p className="mt-1 wrap-break-word text-xs text-muted-foreground">“{value.session.title}” {t("layout.deleteDescription")}</p></div><div className="grid grid-cols-2 gap-2"><button className="h-8 rounded border border-border text-xs" onClick={onDismiss} type="button">{t("layout.cancel")}</button><button className="h-8 rounded bg-destructive text-xs text-destructive-foreground" onClick={() => { onDelete(value.session); onDismiss(); }} type="button">{t("layout.delete")}</button></div></div></div>;
}
