...
import { CheckCircle2, CircleDot, Clock3, FileText, GitBranch, PanelRightClose, PanelRightOpen, ScrollText } from "lucide-react";
...
type InfoTab = "agent" | "files" | "changes" | "logs";
const tabs: Array<{ key: InfoTab; labelKey: string }> = [
  { key: "agent", labelKey: "layout.infoTab.agent" }, { key: "files", labelKey: "layout.infoTab.files" }, { key: "changes", labelKey: "layout.infoTab.changes" }, { key: "logs", labelKey: "layout.infoTab.logs" },
];
...
function LogCard({ entry }: { entry: SessionLogEntry }) {
  const { i18n } = useTranslation();
  return (
    <article className="rounded border border-border bg-background p-2 text-xs">
      <div className="flex items-center justify-between gap-2">
        <span className={cn("font-semibold uppercase", entry.level === "error" && "text-destructive", entry.level === "warn" && "text-primary")}>{entry.level}</span>
        <time className="shrink-0 text-muted-foreground">{new Intl.DateTimeFormat(i18n.language, { timeStyle: "medium" }).format(new Date(entry.timestamp))}</time>
      </div>
      <p className="mt-1 truncate text-muted-foreground">{entry.category}</p>
...
export function SessionInfoPanel({ activeSession, collapsed, onCollapsedChange }: { activeSession: Session | null; collapsed: boolean; onCollapsedChange: (collapsed: boolean) => void }) {
  ...
  const logs = useQuery({ enabled: Boolean(sessionId), queryKey: ["session-workspace", "logs", sessionId, "session.agent_terminal"], queryFn: () => agentService.listSessionLogs({ sessionId: sessionId ?? "", levels: ["error", "warn", "info", "debug"], search: "session.agent_terminal", limit: 8 }) });
  ...
  return <>
    <aside className={cn("ucd-panel min-w-0 overflow-hidden rounded-lg transition-[opacity,transform] duration-200 max-[900px]:hidden", collapsed ? "pointer-events-none translate-x-2 opacity-0" : "opacity-100")}>
      <div className="flex h-full min-h-0 flex-col p-3">
        ...
        <div className="ucd-segmented mb-3 grid grid-cols-4 gap-1 rounded-md p-1">{tabs.map((tab) => <button className={cn("h-8 rounded-md text-xs", activeTab === tab.key ? "bg-background font-semibold text-primary shadow-sm" : "text-muted-foreground hover:bg-muted")} key={tab.key} onClick={() => setActiveTab(tab.key)} type="button">{t(tab.labelKey)}</button>)}</div>
        <div className="min-h-0 flex-1 overflow-y-auto pr-1">
          ...
          <Pane active={activeTab === "logs"}><div className="grid gap-2">{logs.data?.items.slice(0, 8).map((entry) => <LogCard entry={entry} key={entry.id} />)}{logs.isLoading ? <p className="text-xs text-muted-foreground">{t("sessionTabs.state.loading")}</p> : null}{!logs.isLoading && !logs.data?.items.length ? <div className="rounded border border-border p-3 text-xs text-muted-foreground"><ScrollText className="mb-2 h-4 w-4 text-primary" />{t("sessionTabs.logs.empty")}</div> : null}</div></Pane>
        </div>
      </div>
    </aside>
