...
export interface AgentRunnerSelection { kind: AgentRunnerKind; targetId?: string | null; targetRevision?: number | null }
export interface AgentRunnerCapabilities {
  interactiveInput: boolean; pty: boolean; cancellation: boolean; inspection: boolean; recovery: AgentRunnerRecovery;
}
export interface AgentRunnerDescriptor {
  selection: AgentRunnerSelection; label: string; hostLabel: string | null; available: boolean;
  unavailableReason: string | null; simulated: boolean; capabilities: AgentRunnerCapabilities;
}
...
export interface AgentRunnerSafeError { code: AgentRunnerErrorCode; message: string }
