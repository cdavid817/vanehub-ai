...
export function SkillToolActions({ onRefresh, tool }: { onRefresh: () => Promise<unknown>; tool: SkillToolRevision }) {
  ...
  return <>
    <div className="mt-3 flex flex-wrap gap-2 border-t border-border pt-3">
      ...
      {tool.trusted ? <Button disabled={blocked} onClick={(event) => setConfirmation({ kind: "revoke", trigger: event.currentTarget })} size="sm" variant="destructive">{t("skills.tools.revokeAction")}</Button> : null}
      ...
      {tool.quarantined ? <Button disabled={blocked} onClick={() => mutation.mutate("recover")} size="sm" variant="outline">{t("skills.tools.recoverAction")}</Button> : <Button disabled={blocked} onClick={(event) => setConfirmation({ kind: "quarantine", trigger: event.currentTarget })} size="sm" variant="destructive">{t("skills.tools.quarantineAction")}</Button>}
    </div>
...
function SkillToolConfirmationDialog({ kind, onClose, onUpdated, returnFocus, tool }: { kind: ConfirmedOperation; onClose: () => void; onUpdated: () => Promise<unknown>; returnFocus: HTMLElement; tool: SkillToolRevision }) {
  ...
  return <ApplicationDialog closeDisabled={mutation.isPending} description={t(`skills.tools.confirm.${kind}.description`)} maxWidth="max-w-lg" onClose={onClose} returnFocus={returnFocus} title={t(`skills.tools.confirm.${kind}.title`)}>
    <div className="space-y-4 text-sm"><p className="break-all rounded-md bg-muted/40 p-3 font-mono text-xs">{tool.canonicalId}<br />{tool.revision}</p>{mutation.isError ? <p className="rounded-md border border-destructive/40 bg-destructive/10 p-3 text-destructive" role="alert">{mutation.error.message}</p> : null}<div className="flex justify-end gap-2 border-t border-border pt-4"><Button disabled={mutation.isPending} onClick={onClose} variant="outline">{t("skills.tools.cancel")}</Button><Button data-dialog-autofocus disabled={mutation.isPending} onClick={() => mutation.mutate()} variant="destructive">{t(`skills.tools.confirm.${kind}.action`)}</Button></div></div>
  </ApplicationDialog>;
}
