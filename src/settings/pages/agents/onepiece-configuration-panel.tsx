...
import { Box, Check, Globe2, KeyRound, LoaderCircle, Pencil, Plus, RefreshCw, ShieldCheck, Trash2 } from "lucide-react";
...
export function OnePieceConfigurationPanel({ onChanged, searchTerm = "", service = defaultAgentService }: { onChanged: () => Promise<void>; searchTerm?: string; service?: AgentService }) {
  ...
  const filteredProfiles = normalizedSearchTerm
    ? overview.profiles.filter((profile) => [profile.name, profile.provider, profile.modelId, profile.baseUrl ?? ""]
  ...
  return (
    <SectionPanel title={t("onepiece.title")} description={t("onepiece.description")}>
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-end">
        <Button onClick={() => setEditingProfile(null)}><Plus className="h-4 w-4" />{t("onepiece.addProvider")}</Button>
        <Button aria-label={t("onepiece.refresh")} disabled={profilesQuery.isFetching} onClick={() => void profilesQuery.refetch()} variant="outline"><RefreshCw className={`h-4 w-4 ${profilesQuery.isFetching ? "animate-spin" : ""}`} />{t("onepiece.refresh")}</Button>
      </div>

      <section aria-label={t("onepiece.status.title")} className="mt-4 rounded-lg border border-border bg-muted/35 px-4 py-3">
      ...
      <section aria-labelledby="onepiece-providers-heading" className="mt-4">
      ...
      {operationError ? <p className="mt-3 rounded-md border p-3 text-sm ucd-status-warning" role="alert">{operationError instanceof Error ? operationError.message : String(operationError)}</p> : null}
      {notice ? <p className="mt-3 rounded-md border p-3 text-sm ucd-status-success" role="status">{notice}</p> : null}
