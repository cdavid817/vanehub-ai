...
export function PromptHookCardList({
  ...
  return (
    <div className="grid gap-4 xl:grid-cols-2">
      {hooks.map((hook) => (
        <section className="ucd-panel rounded-lg p-4" key={hook.id}>
          <div className="mb-3 flex items-start justify-between gap-3">
            <div className="min-w-0">
              <h3 className="truncate font-semibold">{hook.name}</h3>
          ...
          <p className="min-h-10 text-sm text-muted-foreground">{hook.description}</p>
          ...
          <div className="mt-4 grid grid-flow-col auto-cols-[minmax(7.5rem,1fr)] gap-2 overflow-x-auto pb-1">
            {agents.map((agent) => (
              <label className="flex min-w-0 items-center gap-2 rounded-md border border-border px-2 py-2 text-sm" key={agent.id}>
                <input
                  className="shrink-0"
