...
> 本文是[多 Agent 系统技术架构](../patterns/multi-agent.md) §5 的展开专篇，介绍 A2A 协议的完整技术体系：数据模型（AgentCard / Task / Message / Artifact）、任务状态机、发现机制、三种异步更新通道、协议绑定、安全模型，以及同时作为 A2A Server 与 Client 的双端集成设计。适用于对外暴露 Agent 能力、委派外部 Agent 任务时参考。
>
