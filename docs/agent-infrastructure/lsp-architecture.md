...
>
> 本文介绍 LSP 的完整技术体系：协议分层与生命周期、能力协商、文本同步模型、语言特性与工作区特性、版本演进，以及 LSP 在 AI 编码 Agent 架构中的角色。适用于为宿主应用引入代码智能（诊断、跳转、补全）、将 LSP 能力封装为 Agent 工具、以及 Language Server 子进程管理的实现参考。
>
