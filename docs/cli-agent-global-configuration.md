...
| Qwen Code | `~/.qwen/.env` and `~/.qwen/settings.json` | Updates only `OPENAI_API_KEY`, `OPENAI_BASE_URL`, and `OPENAI_MODEL` in `.env`; an API-key profile also sets `security.auth.selectedType` to `openai` in `settings.json` (an earlier OAuth selection there would otherwise override `.env`), leaving every other setting untouched. A preserve-official profile never writes `settings.json`. |
| iFlow CLI | `~/.iflow/settings.json` | Updates only the owned root-level keys — `selectedAuthType` (`openai-compatible`), `apiKey`, `baseUrl`, `modelName` — and preserves everything else, including identifiers iFlow writes for itself. The key is materialized here because iFlow reads it from nowhere else. |
