...
Installing, upgrading, and detecting CLIs; configuring launch parameters per Agent; pointing an Agent at a provider and model; and SDK dependencies.

Local extensions and product integrations are in [Local extensions](extensions.md); MCP and Prompt Hooks each have their own chapter.
...
- **Desktop only.**
- **Drift is reported, never repaired automatically** — when configuration is changed from outside, you decide what happens.
- **CLI parameters do not rewrite a CLI's own configuration file**; binding works through launch parameters and the relay. **Agent configuration is the exception**, and rewrites the fields it manages with the semantics described above.
