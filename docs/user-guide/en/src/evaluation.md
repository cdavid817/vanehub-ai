...
**Settings → Agent Evaluation** runs multiple Agents against **the same task, on the same starting code**, then lines the results up side by side.
...
1. Open **Settings → Agent Evaluation** (also reachable from the top-bar search).
