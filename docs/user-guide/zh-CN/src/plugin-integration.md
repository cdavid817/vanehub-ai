...
**实时就绪检测需要桌面运行时**。在浏览器预览（Web/mock）里，页面显示的是确定性的模拟数据，状态恒为**不可用**，界面会明确提示：

> 实时插件就绪检测需要桌面运行时。浏览器预览仅显示确定性的模拟数据。

换句话说，**Web 模式下看到的状态不构成任何本机事实**——它没有执行过 `gh`，也没有读过你的凭据。

## 注意事项与限制

- **不安装第三方插件包**，集成由 VaneHub AI 内置，不能自行添加。
- **仅桌面端可执行真实检测**，Web/mock 只有确定性模拟数据。
