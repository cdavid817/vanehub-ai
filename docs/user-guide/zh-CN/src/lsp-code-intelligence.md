...
语言服务器协议（LSP）集成让原生 API Agent 可以向本地语言服务器发起只读查询——定义、引用、悬停信息、诊断，以及另外五类需要服务端支持的能力，完整清单见[支持的服务器与工具](#支持的服务器与工具)。该功能默认停用；必须先启用对应语言，并为每个本地工作区显式授予信任。入口在**设置 → 代码智能**。

![设置中的语言服务器智能页面，显示配置、服务器发现、启动参数与受信任的工作区](assets/screenshots/settings-code-intelligence-zh-CN.png)
...
2. **`jdtls` 本身。** 在 Java 卡片上按**安装服务器**，由 VaneHub AI 下载并解压；或者把你自己解压好的目录填进**服务器安装目录**。

#### 让 VaneHub AI 安装

**安装服务器**会通过 HTTPS 从 `download.eclipse.org` 取回 Eclipse JDT Language Server，解压到 VaneHub AI 自己的应用数据目录，并在卡片上报告结果。**移除服务器**只删除这一份副本，不动别的。

按下之前，卡片会写明这次下载**未做校验和验证**——这句话值得看懂，而不是划过去。VaneHub AI 确实会检查地址在允许列表内、连接是 HTTPS 且不会重定向到列表之外、下载与解压都不超过大小和时间上限。它没做的是把字节对照 Eclipse 发布的摘要校验，因为该项目并未以 VaneHub AI 能定位到的形式为 `latest` 归档发布摘要。所以你信任的是 Eclipse 自己的主机，以及你到它的那条 TLS 连接——和你在浏览器里下载它时给出的信任一样多，不多也不少。如果你不愿给出这份信任，就自己解压一份，改用目录字段；本页不认为哪条路更正当。

用 `latest` 而不是钉死版本还带来两件事：你拿到的东西会随时间变化，且没有升级按钮。重新安装会替换掉已有的那份。

#### 指向你自己的副本

填的目录必须是包含 `plugins/` 与 `config_win` / `config_mac` / `config_linux` 的那一层。带版本号的 launcher 由 VaneHub AI 自己在 `plugins/` 里找，你不用输入版本号。

**你填的目录始终优先于 VaneHub AI 装的那一份。** 两者同时存在没有问题——安装不会把你改指到别处，移除 VaneHub AI 的副本也不会碰你的。

如果 Java 显示为不可用，原因会指明该修下面哪一件：

| 提示 | 该做什么 |
| --- | --- |
| 该服务器所需的运行时未安装 | 装一个 JDK 17+，并确认 `java` 能在你的 shell 里跑起来 |
| 尚未配置服务器安装目录 | 按**安装服务器**，或者把服务器安装目录填上 |
| 该目录中没有服务器启动器 | 指错层级了，或者归档没有完整解压 |
| 该目录中有多个服务器启动器 | 一个目录里混了两个 `jdtls` 版本；重新解压一份干净的 |
| 下载或归档被某项安全限制拒绝 | 归档超过了上限，或含有 VaneHub AI 不会解压的条目；改为自己安装 |
| 下载或解压失败 | 通常是网络或代理。重试是安全的——失败的安装不会留下任何东西 |
| 下载超出了时间上限 | 归档约 50 MB，上限是十分钟，链路慢或被限速就跑不完。重试，或者自己装 |
...
9. 为该本地工作区打开原生 API Agent 会话。无需启用代码索引，上表的九个 LSP 工具也可以生效。
