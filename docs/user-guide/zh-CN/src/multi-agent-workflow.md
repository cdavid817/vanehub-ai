...
- **角色分工**——让不同 Agent 各司其职（架构师定方案、实现者写代码、审查者把关），上下文不用你手工搬来搬去
- **跨模型族协作**——让 Anthropic、OpenAI、Google 等不同厂商的 Agent 在一个任务里接力，互相补盲区
- **多视角评审**——一个 Agent 产出，另一个不同模型族的 Agent 复审，避免单一模型的盲点
- **交还人类**——Agent 在关键节点用 `@用户` 把决定权交回给你

**和单 Agent 的区别**：单 Agent 是一个主体从头干到尾；多 Agent 是多个主体在同一上下文里接力。群聊是单 Agent 会话的超集——只有一个席位的"群聊"行为上等同于单 Agent 会话（详见下文[群聊 vs 单 Agent 会话](#群聊-vs-单-agent-会话)）。
...
**这条链里发生了什么**：三个不同模型族的 Agent 在同一上下文里接力，没人需要你把上一轮对话复制粘贴过去——共享对话线索 + 席位简报让每个人场者都知道前面发生了什么。链深计数 `交接 1/15` 实时显示，触顶会显式终止而非失控。
