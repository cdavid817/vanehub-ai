...
**状态：已实现——桌面端与 Web/mock 界面一致；Web/mock 不落 SQLite，刷新页面即清空。**

## 功能概述

会话、计划、定时任务各有各的列表，人工待办则根本没地方记。任务看板把这四类东西收进同一块看板：**你手写的待办和 Agent 产生的工作并排排列**，用同一套阶段、优先级和筛选来组织。

关键设计是**看板阶段与来源状态彻底分开**：卡片在哪一列由你决定，来源跑到哪一步由运行时投影。两者互不干扰——一个计划跑完了，卡片不会自己跳到「已完成」。
...
- **子会话不单独建卡**。为计划尝试或定时任务运行创建的会话，会作为所属工作项下的活动出现，而**不会**变成一张独立的顶层卡片。否则一个计划跑十轮就会刷出十张卡。
- **归档过的工作项不会被对账重建**。你归档掉的东西，下次对账仍然认它，不会冒出一张替代卡。
- **一张卡可以挂多个来源**。同时关联了会话和计划的卡片会同时命中「会话」和「计划」两个来源筛选，但**它始终只是一张卡**，不会被筛成两份。
...
**这三个操作都不动来源**。永久删除只删掉工作项本身和它的关联记录，被关联的会话、计划、计划运行、定时任务**原封不动**。看板是组织层，不是这些东西的所有者。

恢复时会回到**归档前的阶段和排序位置**，不会掉回收件箱。

## 注意事项与限制

- **Web/mock 不落 SQLite**。创建、对账、筛选、移动、归档的行为都在，但刷新页面就没了。
