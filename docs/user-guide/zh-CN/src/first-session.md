...
勾选**保存为 SSH 连接**可以把这次填的信息存下来复用。首次连接会要求你确认主机密钥，细节见[远程工作区与 SSH](remote-workspaces.md)。
...
| **变更** | Agent 改了哪些文件 |
| **文件** | 工作区文件浏览，标签内有资源管理器/文档视图切换，分别浏览文件和文档 |
| **报告** | 会话报告 |

另外四个能力——**终端记录**、**Shell**、**日志**、**链路**——在独立的**运行时面板**里，从标签条右侧的按钮打开（完整说明见[用户界面](user-interface.md#工作区标签页与运行时面板)）。链路是执行链路追踪，见[可观测性与日志](observability.md)。

**「终端记录」和「Shell」不是一回事**：前者是 Agent 干了什么的记录，后者是给你自己敲命令的终端。

标签页上的数字角标表示记录条数。首版采用有界加载，数据量大时可能只显示部分结果，界面会提示。各标签页的完整功能（Diff 视图、日志搜索、链路 Span 树等）见[会话工作区](session-workspace.md)。
