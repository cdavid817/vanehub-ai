...
![会话工作区，顶部是主标签条，右侧信息面板](assets/screenshots/session-workspace-zh-CN.png)

界面分三块：左侧**会话列表**、中间**工作区**、右侧**信息面板**（会话、CLI 工具、运行状态、本次模型、工作区路径）。

顶部是四个主标签，另有一个独立的**运行时面板**装另外四个：

| 标签 | 用途 |
| --- | --- |
| **工作区** | 主界面，与 Agent 对话并查看其 CLI 终端 |
| **变更** | 本次会话产生的 Git 变更 |
| **文件** | 浏览工作目录；标签内有资源管理器/文档视图切换 |
| **报告** | Token 用量与工具排行 |
| **终端记录**（运行时面板） | Agent 的工具执行记录 |
| **Shell**（运行时面板） | 独立的交互式终端 |
| **日志**（运行时面板） | 会话日志（可搜索、可按时间定位） |
| **链路**（运行时面板） | 执行追踪 |

运行时面板从主标签条右侧的按钮打开，或在有内容指向它的四个能力之一时自动打开。完整说明见[用户界面](user-interface.md#工作区标签页与运行时面板)。
