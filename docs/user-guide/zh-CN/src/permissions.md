...
Gemini CLI、Codex CLI 与 OpenCode 则把模板投影成各自的原生审批/沙箱启动参数，表达方式各不相同——OpenCode 用环境变量，Codex CLI 用命令行选项。你不需要关心这些差异，配好模板即可。

**Antigravity CLI 没有可投影的原生参数。** 它仍然受 VaneHub AI 这一层的策略约束（审批卡片、审计记录照常），但不会像上面三个那样在自己的进程里再执行一遍同等限制。策略在哪一层生效，取决于每个 Agent 声明的能力，而不是所有 CLI 都一样。
