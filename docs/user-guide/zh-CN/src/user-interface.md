...
左侧活动栏的 **Plan 执行**进入计划中心，做的事是「**用 OnePiece 分解、审批、执行并验证任务**」。

![计划中心，从目标开始生成任务图](assets/screenshots/plan-center-zh-CN.png)

**它和 Loop 的分工**：Loop 是让一个 Agent 对着验证命令反复迭代到通过；Plan 是先把目标拆成一张有边界的任务图，**你逐条审批之后**才开始执行。

流程分三步：

### 1. 描述目标

填**目标**、**项目路径**、**基准引用**三项。目标里要写清期望结果、约束条件和重要的验收要求——OnePiece 据此生成任务图。

### 2. 审批任务图

**执行前你可以改任何一项**：任务标题与描述、验收标准、资源限制、顺序与依赖。
...
| --- | --- |
| 任务数 | 1 到 10 个 |
| 每个任务的验收标准 | 1 到 3 条，且每条都要绑定证据（自动或人工） |
| 每个任务的验证命令 | 至少一条必需命令 |
| 最终验证 | 至少一条必需的最终验证命令 |
| 最大尝试次数 | 1 到 5 |
| 依赖 | 必须连接两个不同且存在的任务，**不能成环** |

**执行与验证策略在批准时被冻结**，包括项目探索开关、每个任务的最大尝试次数、自动修复分类。批准按钮叫**审批并启动**，点下去就从计划模式切到 Agent 模式。

> **执行使用保留的 worktree，VaneHub 绝不会自动提交、合并、推送或删除它**。结果需要你自己检视后处理。

### 3. 运行与验收

运行视图按任务展示进度（`已验证 m / n 个任务`）、每次尝试的 token 与工具调用数、变更了多少文件、错误分类，以及**集成最终验证**和它的修复历史。

运行控件：**暂停 / 继续 / 取消 / 恢复 / 重试 / 重试最终验证 / 返回规划 / 接受结果**。

斜杠命令 `/plans` 可直接打开计划中心，见[斜杠命令](slash-commands.md)。目标层面的追踪见[目标管理](goal-management.md)。
