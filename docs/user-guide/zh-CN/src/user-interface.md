...
会话列表左侧的活动栏只有四个入口：上组是**会话**、**收件箱**、**自动化**，底部是**设置**。在没有文本框获得焦点时，`Ctrl+1` 到 `Ctrl+4`（macOS 为 `⌘1` 到 `⌘4`）可以在它们之间切换。

- **收件箱**回答「此刻哪些事需要我」：**需要关注**、**运行中**、**最近完成**三段把任务控制台的运行与未读系统活动合在一起，也是唯一带未读角标的入口。顶部的**列表 / 看板**切换可以换成任务看板；底部两个折叠区分别展开完整的**任务控制台**与**活动日志**时间线。
- **自动化**把无人值守运行的东西收成三个页签：**循环工程**、**定时任务**、**目标中心**。入口会记住你上次用的页签。
- **设置**进入设置中心；**使用文档**固定在其侧栏底部，**Agent 评测**位于其中的 Agent 分组。

![收件箱的需要关注、运行中与最近完成三段](assets/screenshots/inbox-zh-CN.png)

旧链接 `/workspace/loops`、`/workspace/work-board`、`/workspace/mission-control` 等仍然可用，会直接打开新的宿主。顶栏的**全局搜索**也会列出看板、循环工程、定时任务、目标中心与 Agent 评测，可直接跳转。
...
**自动化 → 循环工程**管理 Loop 工程：运行列表与检视、运行控件（暂停/继续/取消/接受/拒绝）、验证命令编辑器、时间线。Loop 的概念与创建见 [Loop Engineering 工程](loop-engineering.md)。
