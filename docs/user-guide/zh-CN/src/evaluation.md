...
**设置 → Agent 评测**在**同一道题、同一份初始代码**上跑多个 Agent，然后把结果并排摆出来比。
...
> **哪些 Agent 能参赛由 Agent 注册表决定，不是写死的清单**。所有未标记 `legacy` 的已注册 Agent 都是候选——OnePiece、五个 headless CLI（例如 OpenCode）以及 `main` 上的六个 ACP Agent。iFlow CLI 标记为 `legacy`（仅终端，没有竞技场可驱动的托管对话），因此永远不会出现。当前检测为**可用**的 Agent 会被预先勾选，最多勾到竞技场上限 8 个；你可以勾选不可用的 Agent，但它的尝试会以「Agent 失败」结束，而不是在开始前被拒绝。逐 Agent 的一行见 [Agent 能力矩阵](../../../reference/agents/capability-matrix.md)。
...
| 结果 | 含义 |
| --- | --- |
| **已排队** / **运行中** | 非终态——只要有尝试处于这两种状态，列表就持续轮询 |
...
- **候选 Agent 来自注册表**：除 `legacy` Agent 之外的全部（所以 OpenCode 是候选，iFlow CLI 不是）。可用的 Agent 会被预先勾选，最多 8 个。
- **单个竞技场最多 8 个尝试**。
