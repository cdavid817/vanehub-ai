...
- **`#[allow(dead_code)]` 一律 item 级（禁止模块级），且注释必须写明是哪个任务让它可达、届时移除。** 本上下文在 Task 12 的 bootstrap 之前没有任何东西被真实构造，所以大量项在此之前都是死代码。两条实测出来的机制，判断归属时必须知道：
  - **被一个仍然死掉的函数调用，不会让被调方变活。** 所以"Task N 会调用它"不等于"Task N 之后可以移除属性"——要看让 *调用方* 变活的是哪个任务。
  - **`#[allow(dead_code)]` 加在 inherent impl 的方法上，会连带屏蔽该方法调用到的一切。** 因此单独摘掉某一项的属性去跑 `cargo check` 会得到"没有告警"的假信号；必须连同其调用方的属性一起摘才能得到可信结论。
  - `unused_imports` 与 `dead_code` 规则不同：前者只看名字有没有被引用，不受可达性约束。
...
const NON_CHAT_KEYWORDS: &[&str] = &[
    // `embed-` 是本次新增的一项（其余沿用 service.rs:71-87 的原表）。少了它，Cohere 的真实 id
    ...
    "embedding", "embed-", "rerank", "whisper", "tts", "audio", "image", "moderation", "realtime",
