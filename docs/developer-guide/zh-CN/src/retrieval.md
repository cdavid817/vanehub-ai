...
- **索引覆盖 ≠ 外发授权**：后台索引源枚举全部有效 active 记录（含 workspace 与受限受众）进入本地 FTS；没有外发授权的记录固定为 `keyword_only` 状态，不入 embedding 队列，worker 在每次 embed 前还会经 `EmbeddingEgressGuardPort` 回源复核。索引状态中的 `keywordOnly` 计数单独报告，不与 pending 混淆。
