...
| **状态/权限** | 无状态,仅参数传递 | 有状态,需鉴权、维护连接 | 纯说明型 Skill 无状态;可执行 Skill 受来源、完整性、权限与沙箱治理 |
| **可移植性** | 依赖具体实现(各家 API 略有差异) | 开放协议,跨客户端通用 | 已开放为标准 |

> 表里的「无状态、纯文本」只描述**纯说明型** Skill——一个只有 `SKILL.md` 的包。Agent Skills 规范同时允许 Skill 携带资源与脚本,VaneHub 的可执行 Skill Tool 因此不走这一栏:它有来源与签名/哈希校验、作用域、权限判定、WebAssembly 沙箱与证据留存,见 [Skill Tool 运行时安全](skill-tool-runtime-security.md)与 [Skill 覆盖层治理](skill-overlay-governance.md)。把「Skill 天生不需要权限系统」当成通则,会漏掉这一整条治理链路。
